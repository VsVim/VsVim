...
type internal NormalMode 
    ...
    member this.BuildOperationsMap = 
        ...
        let completeOps : seq<KeyInput * (int -> Register -> unit)> = seq {
            ...
            yield (KeyInput('u', KeyModifiers.Control), (fun count _ -> _operations.ScrollLines ScrollDirection.Up count))
            yield (KeyInput('d', KeyModifiers.Control), (fun count _ -> _operations.ScrollLines ScrollDirection.Down count))
