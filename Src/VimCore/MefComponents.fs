...
type internal TrackingLineColumn(_textBuffer: ITextBuffer, _offset: int, _mode: LineColumnTrackingMode, _onClose: TrackingLineColumn -> unit) =
    ...
    member x.OnBufferChanged(e: TextContentChangedEventArgs) =
        match _line with
        | Some snapshotLine ->
            if e.AfterVersion <> snapshotLine.Snapshot.Version then x.AdjustForChange snapshotLine e
    ...
    member x.AdjustForChange (oldLine: ITextSnapshotLine) (e: TextContentChangedEventArgs) =
        ...
        | LineColumnTrackingMode.LastEditPoint ->
            if x.IsLineDeletedByChange oldLine changes then _offset <- 0
    ...
    member x.AdjustForChangeCore (oldLine: ITextSnapshotLine) (e: TextContentChangedEventArgs) =
        ...
        if number >= 0 && number < newSnapshot.LineCount
        then _line <- Some(newSnapshot.GetLineFromLineNumber number)
        else x.MarkInvalid oldLine
    ...
    member x.IsLineDeletedByChange (snapshotLine: ITextSnapshotLine) (changes: INormalizedTextChangeCollection) =
        ...
        if changes.IncludesLineChanges then
            ...
            for change in changes do
                if change.LineCountDelta < 0 && change.OldSpan.Contains span then isDeleted <- true
    ...
    interface ITrackingLineColumn with
        ...
        member x.Column =
            match x.Point with
            | Some p -> Some(SnapshotColumn(p))
            | None -> None

        member x.VirtualColumn =
            match x.VirtualPoint with
            | Some p -> Some(VirtualSnapshotColumn(p))
            | None -> None
...
type internal TrackingVisualSpan =
    ...
    member x.TextBuffer = x.TrackingLineColumn.TextBuffer
    ...
    member x.VisualSpan =
        ...
        | Some point ->
            ...
            | Line(_, count) ->
                ...
                SnapshotLineRangeUtil.CreateForLineAndMaxCount line count |> VisualSpan.Line
    ...
    static member Create (bufferTrackingService: IBufferTrackingService) visualSpan =
        ...
        | VisualSpan.Character characterSpan ->
            ...
            TrackingVisualSpan.Character
                (trackingLineColumn, characterSpan.LineCount, characterSpan.LastLineMaxPositionCount)
        ...
        | VisualSpan.Line snapshotLineRange ->
            ...
            let trackingLineColumn =
                bufferTrackingService.CreateLineOffset textBuffer snapshotLineRange.StartLineNumber 0
                    LineColumnTrackingMode.Default
        ...
        | VisualSpan.Block blockSpan ->
            ...
            TrackingVisualSpan.Block
                (trackingLineColumn, blockSpan.TabStop, blockSpan.SpacesLength, blockSpan.Height, blockSpan.EndOfLine)
...
type internal TrackingVisualSelection(_trackingVisualSpan: ITrackingVisualSpan, _path: SearchPath, _column: int, _blockCaretLocation: BlockCaretLocation) =
...
type internal BufferTrackingService() =
    ...
    member x.OnBufferChanged(e: TextContentChangedEventArgs) =
        ...
        | Some trackedData ->
            trackedData.List |> Seq.iter (fun trackingLineColumn -> trackingLineColumn.OnBufferChanged e)
        | None -> ()
    ...
    member x.HasTrackingItems textBuffer = x.FindTrackedData textBuffer |> Option.isSome

    interface IBufferTrackingService with
        member x.CreateLineOffset textBuffer lineNumber offset mode =
            x.CreateLineOffset textBuffer lineNumber offset mode :> ITrackingLineColumn
        ...
        member x.CreateVisualSelection visualSelection =
            TrackingVisualSelection.Create x visualSelection :> ITrackingVisualSelection
...
type internal ChangeTracker [<ImportingConstructor>] (_vim: IVim) =
    ...
    member x.StoreCommand currentCommand =
        ...
        | Some lastCommand ->
            ...
            if shouldLink
            then _vimData.LastCommand <- StoredCommand.LinkedCommand(lastCommand, currentCommand) |> Some
            else _vimData.LastCommand <- Some currentCommand
...
type internal ProtectedOperations =
    ...
    new() = { _errorHandlers = Seq.empty |> GenericListUtil.OfSeq }
    ...
    member x.AlertAll e =
        ...
        for handler in x._errorHandlers do
            ...
            with e -> Debug.Fail((sprintf "Error handler threw: %O" e))
...
type internal VimSpecificServiceHost [<ImportingConstructor>] (_vimHost: Lazy<IVimHost>, [<ImportMany>] _services: Lazy<IVimSpecificService> seq) =
...
type internal VimWordCompletionSessionFactoryService [<ImportingConstructor>] (_vimSpecificServiceHost: IVimSpecificServiceHost) =
    ...
    interface IWordCompletionSessionFactoryService with
        member x.CreateWordCompletionSession textView wordSpan words isForward =
            x.CreateWordCompletionSession textView wordSpan words isForward
        [<CLIEvent>]
...
type internal SingleSelectionUtil(_textView: ITextView) =
    ...
    member x.SetSelectedSpans(selectedSpans: SelectionSpan seq) =
        ...
        if selectedSpan.Length <> 0 then _textView.Selection.Select(selectedSpan.AnchorPoint, selectedSpan.ActivePoint)
...
type internal SingleSelectionUtilFactory() =
    ...
    member x.GetSelectionUtil(textView: ITextView) =
        ...
        propertyCollection.GetOrCreateSingletonProperty
            (s_key, (fun () -> SingleSelectionUtil(textView) :> ISelectionUtil))
...
type internal SelectionUtilService [<ImportingConstructor>] (_vimSpecificServiceHost: IVimSpecificServiceHost) =

    static let s_singleSelectionUtilFactory = SingleSelectionUtilFactory() :> ISelectionUtilFactory
