...
type VimCharSet
    (
        _text: string,
        _partList: ReadOnlyCollection<VimCharSetPart>
    ) =
    ...
    member x.Contains c = 
        ...
        for part in _partList do
            match part with
            | VimCharSetPart.Char (partChar, exclude) when partChar = c -> 
                contains <- not exclude
            | VimCharSetPart.CharRange (firstPartChar, secondPartChar, exclude) when c >= firstPartChar && c <= secondPartChar ->
                contains <- not exclude
            | VimCharSetPart.Letters exclude when CharUtil.IsLetter c -> 
                contains <- not exclude
            | _ -> ()
        contains
    ...
    static member TryParse(text: string): VimCharSet option =
        let charAt index =
            if index < text.Length then Some (text.[index])
            else None
        ...
        let parsePartCore startIndex exclude = 
            ...
            let resolveSingleChar c =
                if c = '@' then VimCharSetPart.Letters exclude
                else VimCharSetPart.Char (c, exclude)
            ...
            | Some (firstPartItem, index) ->
                ...
                | Some '-' when index + 1 < text.Length -> 
                    ...
                    | None -> None
                    | Some (secondPartItem, index) -> Some (VimCharSetPart.CharRange(firstPartItem, secondPartItem, exclude), index)
                | _ -> None

        // This handles parsing out a full part including the exclusion
        let parsePart startIndex =
            Debug.Assert(startIndex < text.Length)
            if startIndex + 1 < text.Length && text.[startIndex] = '^' then parsePartCore (startIndex + 1) true
