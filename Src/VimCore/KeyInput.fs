...
[<Sealed>]
type KeyInput(_key: VimKey, _modKey: VimKeyModifiers, _literal: char option) =

    member x.Char =
        match _literal with
        | Some c -> c
        | None -> CharUtil.MinValue
    ...
    member x.CompareTo(right: KeyInput) =
        if obj.ReferenceEquals(right, null) then
            ...
            if comp <> 0 then
                ...
                if comp <> 0 then comp else compare left.Key right.Key
    ...
    static member op_Equality (this, other) =
        System.Collections.Generic.EqualityComparer<KeyInput>.Default.Equals(this, other)
    static member op_Inequality (this, other) =
        not (System.Collections.Generic.EqualityComparer<KeyInput>.Default.Equals(this, other))
...
[<Sealed>]
type KeyInputData(_keyInput: KeyInput, _wasMapped: bool) =
    ...
    override x.ToString() = System.String.Format("{0}:{1}", x.KeyInput, x.WasMapped)
...
module KeyInputUtil =
    ...
    let ControlCharToKeyInputMap =
        ...
        Seq.append alpha other |> Map.ofSeq

    /// This is the set of predefined KeyInput values that Vim cares about
    let VimKeyInputList =

        let rawSeq = RawCharData |> Seq.map (fun c -> KeyInput(VimKey.RawCharacter, VimKeyModifiers.None, Some c))

        let standardSeq = VimKeyRawData |> Seq.map (fun (key, charOpt) -> KeyInput(key, VimKeyModifiers.None, charOpt))
    ...
    let VimKeyCharList = VimKeyCharSet |> Set.toList
    ...
    let CharToKeyInputMap =
        ...
        for tuple in inputs do
            ...
            if Option.isSome found then System.Diagnostics.Debug.Fail("This is the failure")
            debugMap <- Map.add c (snd tuple) debugMap
#endif

        let map = inputs |> Map.ofSeq
        if map.Count <> inputs.Length then failwith Resources.KeyInput_DuplicateCharRepresentation
        map
    ...
    let ChangeKeyModifiersDangerous (ki: KeyInput) keyModifiers = KeyInput(ki.Key, keyModifiers, ki.RawChar)
    ...
    let ApplyKeyModifiers (keyInput: KeyInput) (targetModifiers: VimKeyModifiers) =
        ...
        let normalizeControl (keyInput: KeyInput) =
            if Util.IsFlagSet keyInput.KeyModifiers VimKeyModifiers.Alt then
                ...
                | Some c ->
                    ...
                    let c =
                        if CharUtil.IsUpperLetter c then CharUtil.ToLower c else c
        ...
        if Util.IsFlagSet targetModifiers VimKeyModifiers.Alt then
            ...
            let keyInput =
                if Util.IsFlagSet targetModifiers VimKeyModifiers.Shift
                then normalizeShift keyInput
                else keyInput

            // Next normalize the control case.
            let keyInput =
                if Util.IsFlagSet targetModifiers VimKeyModifiers.Control
                then normalizeControl keyInput
                else keyInput
    ...
    let NormalizeKeyModifiers(keyInput: KeyInput) =
        ...
        | VimKey.RawCharacter, Some c -> ApplyKeyModifiersToChar c keyInput.KeyModifiers
        | key, _ -> ApplyKeyModifiersToKey key keyInput.KeyModifiers
    ...
    let IsCore(keyInput: KeyInput) =
        if Util.IsFlagSet keyInput.KeyModifiers VimKeyModifiers.Alt then
            ...
            // Any standard vim key with or without modifiers is a core key input.
            true
