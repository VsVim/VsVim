...
type internal SettingsMap(_rawData: Setting seq) as this =
    ...
    do _rawData |> Seq.iter (fun setting -> this.AddSetting setting)
...
type internal GlobalSettings() =
    ...
    static let GlobalSettingInfoList =
        [| (AtomicInsertName, AtomicInsertName, SettingValue.Toggle false, SettingOptions.None)
           ...
           (ShellName, "sh",
            "ComSpec"
            |> SystemUtil.GetEnvironmentVariable
            |> SettingValue.String, SettingOptions.FileName)
    ...
    static let GlobalSettingList =
        ...
        |> Seq.map (fun (name, abbrev, defaultValue, options) ->
            { Name = name
              Abbreviation = abbrev
              LiveSettingValue = LiveSettingValue.Create defaultValue
              IsGlobal = true
    ...
    static member DisableAllCommand =
        KeyInputUtil.ApplyKeyModifiersToKey VimKey.F12 (VimKeyModifiers.Control ||| VimKeyModifiers.Shift)
    ...
    member x.AddCustomSetting name abbrevation customSettingSource =
        ...
        let setting =
            { Name = name
              ...
              IsGlobal = true
              SettingOptions = SettingOptions.None }
    ...
    member x.SetCommaOptions name mappingList options testFunc =
        let settingValue =
            ...
            |> Seq.map (fun (name, value) ->
                if testFunc options value then Some name else None)
    ...
    member x.GetPathOptionList text =
        ...
        while i < length do
            match text.[i] with
            | '\\' ->
                if i + 1 < length then builder.AppendChar text.[i + 1]
                i <- i + 2
            | ',' ->
        ...
        if builder.Length > 0 then addOne (builder.ToString())
    ...
    interface IVimGlobalSettings with
        ...
        member x.AddCustomSetting name abbrevation customSettingSource =
            x.AddCustomSetting name abbrevation customSettingSource
        ...
        member x.ClipboardOptions
            with get () =
                x.GetCommaOptions ClipboardName ClipboardOptionsMapping ClipboardOptions.None (fun x y -> x ||| y)
        ...
        member x.IsIdentCharSet
            with get () =
                match VimCharSet.TryParse(_map.GetStringValue IsIdentName) with
                | Some s -> s
                | None -> SettingsDefaults.IsIdentCharSet
        ...
        member x.KeyModelOptions
            with get () =
                x.GetCommaOptions KeyModelName KeyModelOptionsMapping KeyModelOptions.None (fun x y -> x ||| y)
        ...
        member x.SelectModeOptions
            with get () =
                x.GetCommaOptions SelectModeName SelectModeOptionsMapping SelectModeOptions.None (fun x y -> x ||| y)
        ...
        member x.IsVirtualEditBlock =
            x.IsCommaSubOptionPresent VirtualEditName "block" || x.IsCommaSubOptionPresent VirtualEditName "all"
        member x.IsVirtualEditInsert =
            x.IsCommaSubOptionPresent VirtualEditName "insert" || x.IsCommaSubOptionPresent VirtualEditName "all"
        member x.IsVirtualEditAll = x.IsCommaSubOptionPresent VirtualEditName "all"
        member x.IsVirtualEditOneMore =
            x.IsCommaSubOptionPresent VirtualEditName "onemore" || x.IsCommaSubOptionPresent VirtualEditName "all"
...
type internal LocalSettings(_globalSettings: IVimGlobalSettings) =
    ...
    member x.IsNumberFormatSupported numberFormat =
        ...
        | NumberFormat.Decimal ->
            ...
            true
        | NumberFormat.Binary -> isSupported "bin"
        | NumberFormat.Octal -> isSupported "octal"
        | NumberFormat.Hex -> isSupported "hex"
        | NumberFormat.Alpha -> isSupported "alpha"

    member x.TrySetValue settingName value =
        if _map.OwnsSetting settingName
        then _map.TrySetValue settingName value
        else _globalSettings.TrySetValue settingName value

    member x.TrySetValueFromString settingName strValue =
        if _map.OwnsSetting settingName
        then _map.TrySetValueFromString settingName strValue
        else _globalSettings.TrySetValueFromString settingName strValue

    member x.GetSetting settingName =
        if _map.OwnsSetting settingName then _map.GetSetting settingName else _globalSettings.GetSetting settingName
    ...
    interface IVimLocalSettings with
        ...
        member x.IsKeywordCharSet
            with get () =
                match VimCharSet.TryParse(_map.GetStringValue IsKeywordName) with
                | Some s -> s
                | None -> SettingsDefaults.IsKeywordCharSet
...
type internal WindowSettings(_globalSettings: IVimGlobalSettings, _textView: ITextView option) as this =
    ...
    interface IVimWindowSettings with
        ...
        member x.GetSetting settingName =
            if _map.OwnsSetting settingName then
                _map.GetSetting settingName
            else
                _globalSettings.GetSetting settingName
...
type internal EditorToSettingSynchronizer [<ImportingConstructor>] () =
            ...
            ({ EditorOptionKey = DefaultTextViewOptions.WordWrapStyleId.Name
               GetEditorValue =
                   (fun editorOptions ->
                       ...
                       | None -> None
                       | Some s ->
                           Util.IsFlagSet s WordWrapStyles.WordWrap
                           |> SettingValue.Toggle
                           |> Option.Some)
    ...
    member x.StartSynchronizing (vimBuffer: IVimBuffer) settingSyncSource =
        ...
        if not (properties.ContainsProperty _key) then
            ...
            | Some modeLine, Some badOption ->
                Resources.Common_InvalidModeLineSetting badOption modeLine |> vimBuffer.VimBufferData.StatusUtil.OnError
            | _ -> ()
    ...
    member x.IsTrackedEditorSetting optionId = _settingList |> Seq.exists (fun x -> x.EditorOptionKey = optionId)
    ...
    member x.CopyVimToEditorSettings vimBuffer =
        x.TrySync vimBuffer (fun vimBuffer editorOptions ->
            for data in _settingList do
                ...
                if value <> null then editorOptions.SetOptionValue(data.EditorOptionKey, value))
    ...
    member x.CopyEditorToVimSettings(vimBuffer: IVimBuffer) =
        x.TrySync vimBuffer (fun vimBuffer editorOptions ->
            for data in _settingList do
                ...
                | None -> ()
                | Some value -> data.SetVimValue vimBuffer value)
