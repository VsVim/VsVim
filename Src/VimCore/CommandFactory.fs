...
type internal CommandFactory(_operations: ICommonOperations, _capture: IMotionCapture) =
    ...
    member x.CreateScrollCommands() = SharedScrollCommands

    member x.CreateSelectionCommands() = SharedSelectionCommands
    ...
    member x.CreateMovementsFromMotions() =
        let processMotionBinding binding =
            ...
            | MotionBinding.Dynamic(name, motionFlags, bindDataStorage) ->
                ...
                let flags =
                    if Util.IsFlagSet motionFlags MotionFlags.HandlesEscape
                    then CommandFlags.Movement ||| CommandFlags.HandlesEscape
                    else CommandFlags.Movement
    ...
    member x.CreateMovementTextObjectCommands() =
        let processMotionBinding (binding: MotionBinding) =
            ...
            | MotionBinding.Dynamic(name, motionFlags, bindDataStorage) ->
                ...
                // where the command will move the motion
                let bindDataStorage =
                    bindDataStorage.Convert(fun motion -> VisualCommand.MoveCaretToTextObject(motion, textObjectKind))
    ...
    member x.CreateMovementCommands() =
        ...
        let taken =
            standard
            |> Seq.map (fun command -> command.KeyInputSet)
            |> Set.ofSeq

        let motion =
            x.CreateMovementsFromMotions() |> Seq.filter (fun command -> not (taken.Contains command.KeyInputSet))
        standard
        |> Seq.append motion
        |> List.ofSeq
