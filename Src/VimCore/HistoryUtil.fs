...
type internal HistorySession<'TData, 'TResult>(_historyClient: IHistoryClient<'TData, 'TResult>, _initialClientData: 'TData, _command: EditableCommand, _localAbbreviationMap: IVimLocalAbbreviationMap, _motionUtil: IMotionUtil) =
    ...
    member x.CreateBindResult() =
        let bindData =
            { KeyRemapMode = _historyClient.RemapMode
              MappedBindFunction = x.Process }
        MappedBindResult<_>.NeedMoreInput bindData

    member x.CreateBindDataStorage() =
        MappedBindDataStorage.Complex(fun () ->
            { KeyRemapMode = _historyClient.RemapMode
              MappedBindFunction = x.Process })
    ...
    member x.ProcessCore(keyInputData: KeyInputData, suppressAbbreviations: bool) =
        ...
        | Some HistoryCommand.Execute ->
            ...
            if not wasMapped then _historyClient.HistoryList.Add _command.Text
            _inPasteWait <- false
        ...
        | Some HistoryCommand.Home ->
            _command.Home() |> x.ResetCommand
            x.CreateBindResult()
        | Some HistoryCommand.End ->
            _command.End() |> x.ResetCommand
            x.CreateBindResult()
        | Some HistoryCommand.Left ->
            _command.Left() |> x.ResetCommand
            x.CreateBindResult()
        | Some HistoryCommand.Right ->
            _command.Right() |> x.ResetCommand
            x.CreateBindResult()
        | Some HistoryCommand.Backspace ->
            ...
            | 0 ->
                ...
                MappedBindResult.Cancelled
            | _ ->
                _command.Backspace() |> x.ResetCommand
                x.CreateBindResult()
        | Some HistoryCommand.Delete ->
            _command.Delete() |> x.ResetCommand
            x.CreateBindResult()
        | Some HistoryCommand.Previous -> x.ProcessPrevious()
        | Some HistoryCommand.Next -> x.ProcessNext()
        | Some HistoryCommand.Paste ->
            _inPasteWait <- true
            x.CreateBindResult()
        | Some(HistoryCommand.PasteSpecial wordKind) -> x.ProcessPasteSpecial wordKind
    ...
    member x.ProcessPrevious() =
        ...
        | HistoryState.Index(list, index) -> x.DoHistoryScroll list (index + 1)
    ...
    member x.ProcessNext() =
        match _historyState with
        | HistoryState.Empty -> _historyClient.Beep()
    ...
    member x.Cancel() = _historyClient.Cancelled _clientData
...
and internal HistoryUtil() =
    ...
    static let _keyInputMap =
        ...
        let set2 =
            ...
            |> Seq.map (fun (name, command) ->
                ...
                let keyInput4 =
                    KeyInputUtil.ApplyKeyModifiers keyInput1 (VimKeyModifiers.Shift ||| VimKeyModifiers.Control)
                seq {
    ...
    static member CreateHistorySession<'TData, 'TResult> historyClient clientData command localAbbreviationMap
                  motionUtil =
        let historySession =
            HistorySession<'TData, 'TResult>(historyClient, clientData, command, localAbbreviationMap, motionUtil)
