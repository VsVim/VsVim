...
type CachedParsedItem<'T> =
    ...
    static member GetItems (snapshot: ITextSnapshot) key doParse =
        ...
        | Some cachedParsedItem ->
            if cachedParsedItem.Version = snapshot.Version.VersionNumber
            then cachedParsedItem.Items
            else parseAndSave()
        | None -> parseAndSave()

/// Used to represent xml / html tags within the file.
type TagBlock(text: string, fullSpan: Span, innerSpan: Span, children: List<TagBlock>) =
...
type TagBlockParser(snapshot: ITextSnapshot) =
    ...
    member x.TestPosition position func =
        if position >= snapshot.Length then false else func (snapshot.[position])

    member x.TestPositionChar position target = x.TestPosition position (fun c -> c = target)

    member x.SkipBlanks position = x.SkipWhile position CharUtil.IsBlank

    member x.SkipWhiteSpace position = x.SkipWhile position CharUtil.IsWhiteSpace
    ...
    member x.ParseName startPosition =
        ...
        if length = 0 then None else Some(Span(startPosition, length))

    /// Parse out the name of an attribute.  Returns the Span of the name if found an None if there is
    /// not a valid name
    member x.ParseAttributeName startPosition = x.ParseName startPosition
    ...
    member x.ParseChar position c =
        if x.TestPositionChar position c then Some(position + 1) else None
        ...
        while not isDone do
            if x.TestPositionChar position '>' then
                ...
                | None ->
                    isError <- true
                    isDone <- true
                | Some span -> position <- x.SkipWhiteSpace span.End

        if isError then None else Some position
        ...
        while position < _snapshot.Length && Option.isNone endPosition do
            ...
            | None ->
                ...
                | Some(endTagName, nextPosition) ->
                    if StringUtil.IsEqualIgnoreCase tagName endTagName
                    then endPosition <- Some(position, nextPosition)
...
module TagBlockUtil =
    ...
    let GetTagBlocks snapshot = CachedParsedItem<TagBlock>.GetItems snapshot _tagBlockKey ParseTagBlocks
...
type internal StringLiteralTestPoint(_point: SnapshotPoint, _line: ITextSnapshotLine, _isInStringLiteral: bool []) =
    ...
    member x.FromPoint(point: SnapshotPoint) =
        if _line.ExtentIncludingLineBreak.Contains(point)
        then StringLiteralTestPoint(point, _line, _isInStringLiteral)
        else StringLiteralTestPoint.CreateFromPoint point

    /// Whether the character at the snapshot point associated with this
    /// test point is part of a string literal
    member x.IsInStringLiteral = _isInStringLiteral.[_point.Position - _line.Start.Position]
    ...
    member private x.FindLiterals() =
        ...
        for point in points do
            ...
            | Some quoteChar ->
                ...
                elif verbatim && c = quoteChar && point.Position + 1 < _line.EndIncludingLineBreak.Position
                     && getNextChar point = quoteChar then
    ...
    member x.GetBlock (blockKind: BlockKind) contextPoint =
        ...
        let findMatched plusChar minusChar =
            let inner point count =
                let count =
                    if isChar minusChar point then count - 1
                    elif isChar plusChar point then count + 1
                    else count
                count = 0, count
            inner
        ...
        let isInStringLiteral (target: SnapshotPoint) (sequence: SnapshotPoint seq) =
            ...
            for point in sequence do
                ...
                if isInStringLiteral then
                    if not wasInStringLiteral then start <- Some point
                    if point = target then
                        if depth <= 0 then result <- true, start else result <- true, None
                    if c = startChar then depth <- depth + 1
                    elif c = endChar then depth <- depth - 1
        ...
        let referencePoint =
            if isChar startChar contextPoint
            then SnapshotPointUtil.AddOneOrCurrent contextPoint
            else contextPoint
        ...
        let referencePoint, referencePointIsInStringLiteral =
            match SnapshotPointUtil.GetContainingLine referencePoint
                  |> SnapshotLineUtil.GetExtent
                  |> SnapshotSpanUtil.GetPoints SearchPath.Forward
                  |> isInStringLiteral referencePoint with
        ...
        let rec filterToContext (sequence: SnapshotPoint seq) =
            ...
            seq {
                for point in sequence do
                    ...
                    testPoint <- testPoint.FromPoint point
                    if testPoint.IsInStringLiteral = referencePointIsInStringLiteral then yield point
            }
    ...
    member x.FindMatchingTokenKindCore lineText column =
        ...
        let findComment() =
            ...
            let indexOf text =
                ...
                | None ->
                    if column > 0
                    then StringUtil.IndexOfStringAt text (column - 1) lineText
                    else None
                | Some index -> result
        ...
        let findDirective() =
            ...
            | None -> None
            | Some directive ->
                if directive.Span.Start = column
                then Some(column, MatchingTokenKind.Directive)
                else None
    ...
    member x.FindMatchingToken point =
        ...
        let findMatchingTokenChar target (blockKind: BlockKind) =
            ...
            | Some(startPoint, endPoint) ->
                let otherPoint =
                    if contextPoint = startPoint then endPoint else startPoint
                ...
                if Option.isNone commentStart && isBegin index then
                    ...
                    if target <= index then
                        ...
                        | None ->
                            ...
                            ()
                        | Some commentStart ->
                            if target < index then found <- Some(index + 1) else found <- Some commentStart
        ...
        let findMatchingDirective (target: int) =
            ...
            while blockIndex < blockList.Count do
                ...
                let index = block.Directives |> Seq.tryFindIndex (fun directive -> directive.Span.Contains target)
                match index with
                | None -> blockIndex <- blockIndex + 1
                | Some index ->
                    if index + 1 = block.Directives.Count
                    then if block.IsComplete then found <- block.Directives.[0].Span |> Some
                    else found <- block.Directives.[index + 1].Span |> Some
        ...
        let line, column =
            ...
            let column =
                if data.IsLineBreak
                then max 0 (line.End.Position - line.Start.Position - 1)
                else data.ColumnNumber
            (line, column)
    ...
    member x.FindUnmatchedToken path kind point count =
        ...
        while e.MoveNext() && Option.isNone found do
            ...
            if c = up then
                ...
                count <- count - 1
                if count = 0 then found <- Some e.Current
            elif c = down then
                depth <- depth - 1
...
type internal MotionUtil(_vimBufferData: IVimBufferData, _commonOperations: ICommonOperations) =
    ...
    member x.GetParagraphs path point = _textObjectUtil.GetParagraphs path point

    member x.GetSentences sentenceKind path point = _textObjectUtil.GetSentences sentenceKind path point

    member x.GetSections sectionKind path point = _textObjectUtil.GetSections sectionKind path point

    member x.SpanAndForwardFromLines (line1: ITextSnapshotLine) (line2: ITextSnapshotLine) =
        if line1.LineNumber <= line2.LineNumber
        then SnapshotSpan(line1.Start, line2.EndIncludingLineBreak), true
    ...
    member x.ApplyStartOfLineOption(motionData: MotionResult) =
        Contract.Assert
            (match motionData.MotionKind with
             | MotionKind.LineWise _ -> true
             | _ -> false)
    ...
    member x.MotionWithVisualSnapshotCore (action: SnapshotData -> 'T) (getMotionResult: 'T -> MotionResult option) =
        ...
        | Some snapshotData ->
            ...
            | None -> VisualMotionResult.FailedNoMotionResult
            | Some motionResult ->
                // Now migrate the SnapshotSpan values down to the edit buffer.  If this mapping can't
                // be done then the motion fails.
                match motionResult.MapSpans
                          (fun s -> BufferGraphUtil.MapSpanDownToSingle _bufferGraph s x.CurrentSnapshot) with
    ...
    member x.MotionWithVisualSnapshot(action: SnapshotData -> MotionResult) =
        ...
        | VisualMotionResult.Succeeded motionResult -> motionResult
        | _ ->
    ...
    member x.MotionOptionWithVisualSnapshot(action: SnapshotData -> MotionResult option) =
        ...
        | VisualMotionResult.Succeeded motionResult -> Some motionResult
    ...
    member x.DisplayLineDown count =
        if TextViewUtil.IsWordWrapEnabled _textView
        then x.DisplayLineMotion count
        else x.LineDown count
    ...
    member x.DisplayLineMotion offset =
        ...
        | Some caretLine, Some targetLine ->
            ...
            let span, isForward =
                if caretPoint.Position < targetPoint.Position
                then SnapshotSpan(caretPoint, targetPoint), true
                else SnapshotSpan(targetPoint, caretPoint), false
    ...
    member x.DisplayLineFirstNonBlank() =
        ...
        | Some caretLine -> TextViewUtil.GetVisibleSpan _textView caretLine |> x.FirstNonBlankCore
        | None -> x.FirstNonBlankOnCurrentLine()
    ...
    member x.LineToLineFirstNonBlankMotion (flags: MotionResultFlags) (startLine: ITextSnapshotLine)
           (endLine: ITextSnapshotLine) =
    ...
    member x.GetQuotedStringData quoteChar =
        ...
        let quotePoints =
            let isEscapeChar c = StringUtil.ContainsChar _localSettings.QuoteEscape c
        ...
        let getStringData (stringSpan: SnapshotSpan) =
            ...
            { LeadingWhiteSpace = leadingSpace
              ...
              TrailingWhiteSpace = trailingSpace }
            |> Some
        ...
        let position =
            ...
            if quoteSpans.Count > 0 && position < quoteSpans.[0].Start.Position
            then quoteSpans.[0].Start.Position
            else position
        ...
        while index < quoteSpans.Count && Option.isNone data do
            ...
            if span.End.Position - 1 = position
            then if index % 2 = 0 then data <- getStringData span
            elif span.Contains(position)
            then data <- getStringData span
    ...
    member x.Mark localMark =
        ...
        | Some virtualPoint ->
            ...
            let markPoint =
                if not _globalSettings.IsVirtualEditOneMore && not (SnapshotPointUtil.IsStartOfLine markPoint)
            ...
            MotionResult.Create
                (span, MotionKind.CharacterWiseExclusive, isForward, motionResultFlags = MotionResultFlags.BigDelete)
            |> Some
    ...
    member x.MarkLine localMark =
        ...
        | Some virtualPoint ->
            ...
            MotionResult.Create
                (range.ExtentIncludingLineBreak, MotionKind.LineWise, isForward, MotionResultFlags.None, column) |> Some
    ...
    member x.UnmatchedToken path kind count =
        ...
        | Some matchingPoint ->
            ...
            // Order the tokens appropriately to get the span
            let span, isForward =
                if x.CaretPoint.Position < matchingPoint.Position
                then SnapshotSpan(x.CaretPoint, matchingPoint), true
                else SnapshotSpan(matchingPoint, x.CaretPoint), false
    ...
    member x.MoveCaretToMouse() =
        ...
        | Some point ->
            ...
            let span, isForward =
                if x.CaretPoint.Position < mousePoint.Position
                then SnapshotSpan(x.CaretPoint, mousePoint), true
                else SnapshotSpan(mousePoint, x.CaretPoint), false
    ...
    member x.AllParagraph count =
        ...
        | head :: tail ->
            ...
            if spanHasContent
            then MotionResult.Create(span, MotionKind.CharacterWiseExclusive, isForward = true) |> Some
            else None
        ...
        | Some firstSpan ->
            ...
            let whiteSpaceAfter =
                ...
                match all
                      ...
                      |> SeqUtil.tryHeadOnly with
                | None -> None
                | Some nextPoint ->
                    if nextPoint.Start.Position = endPoint.Position
                    then None
                    else SnapshotSpan(endPoint, nextPoint.Start) |> Some
    ...
    member x.AllSentence count =
        ...
        let sentences =
            x.GetSentences sentenceKind SearchPath.Forward searchColumn
            |> Seq.truncate count
            |> List.ofSeq
        ...
        let span =
            ...
            | head :: tail ->
                ...
                let whiteSpaceAfter =
                    ...
                    if column.IsEndColumn || span.End.Position = column.StartPoint.Position
                    then None
                    else Some column
                ...
                let includePrecedingWhiteSpace() =
                    ...
                    let mutable before =
                        if column.IsStartColumn then column else column.Subtract 1
    ...
    member x.CharSearchCore c count charSearch direction isRepeated =
        ...
        let forward() =
            ...
            if startPoint.Position < endPoint.Position then
                ...
                let startPoint =
                    if repeatTillAlwaysMoves && charSearch = CharSearchKind.TillChar && isRepeated
                       && startPoint.Position < endPoint.Position then
        ...
        let backward() =
            ...
            let endPoint =
                if repeatTillAlwaysMoves && charSearch = CharSearchKind.TillChar && isRepeated
                   && endPoint.Position > startPoint.Position then
    ...
    member x.WordForward kind count motionContext =
        ...
        let count =
            if SnapshotPointUtil.IsWhiteSpace x.CaretPoint && not (SnapshotPointUtil.IsInsideLineBreak x.CaretPoint)
            then count - 1
            else count
        ...
        let endPoint =
            ...
            | MotionContext.Movement -> SnapshotPointUtil.GetPointOrEndPointOfLastLine endPoint
            | MotionContext.AfterOperator ->
                ...
                if isFirstNonBlank && endLine.LineNumber > x.CaretLine.LineNumber then
                    let previousLine =
                        ...
                        |> Seq.skipWhile (fun textLine ->
                            SnapshotLineUtil.IsBlank textLine && textLine.LineNumber > x.CaretLine.LineNumber)
                        |> SeqUtil.tryHeadOnly
        ...
        let span = SnapshotSpan(x.CaretPoint, endPoint)
        MotionResult.Create
            (span, MotionKind.CharacterWiseExclusive, isForward = true, motionResultFlags = MotionResultFlags.AnyWord)
    ...
    member x.WordBackward kind count =
        ...
        MotionResult.Create
            (span, MotionKind.CharacterWiseExclusive, isForward = false, motionResultFlags = MotionResultFlags.AnyWord)
    ...
    member x.EndOfWord kind count =
        ...
        let searchPoint =
            ...
            | None -> x.CaretPoint
            | Some span ->
                if SnapshotSpanUtil.IsLastIncludedPoint span x.CaretPoint
                then span.End
                else x.CaretPoint
    ...
    member x.EndOfLine count =
        ...
        MotionResult.Create
            (span, MotionKind.CharacterWiseInclusive, isForward = true, motionResultFlags = MotionResultFlags.EndOfLine)

    /// Find the first non-whitespace character on the current line.
    member x.FirstNonBlankOnCurrentLine() = x.FirstNonBlankCore x.CaretLine.Extent
    ...
    member x.FirstNonBlankCore span =
        ...
        let startPoint, endPoint, isForward =
            if start.Position <= target.Position then start, target, true else target, start, false
    ...
    member x.TagBlock count point kind =
        ...
        | Some tagBlock ->
            let span =
                ...
                | TagBlockKind.Inner ->
                    if tagBlock.InnerSpan.IsEmpty && point.Position = tagBlock.InnerSpan.Start
                    then tagBlock.FullSpan
                    else tagBlock.InnerSpan

            let span = SnapshotSpan(x.CurrentSnapshot, span)
            MotionResult.Create
                (span, MotionKind.CharacterWiseExclusive, isForward = true,
                 motionResultFlags = MotionResultFlags.SuppressAdjustment) |> Some
    ...
    member x.GetExpandedTagBlock startPoint (endPoint: SnapshotPoint) kind: SnapshotSpan option =
        ...
        | Some tagBlock ->
            let span =
                match kind with
                | TagBlockKind.All ->
                    if tagBlock.InnerSpan.Contains startPoint.Position
                    then Some tagBlock.FullSpan
                    else tagBlock.Parent |> Option.map (fun t -> t.FullSpan)
                | TagBlockKind.Inner ->
                    if tagBlock.InnerSpan.Contains startPoint.Position then
                        if tagBlock.InnerSpan = Span.FromBounds(startPoint.Position, endPoint.Position)
                        then Some tagBlock.FullSpan
                        else Some tagBlock.InnerSpan
                    else
                        tagBlock.Parent
                        |> Option.map (fun t ->
                            if t.InnerSpan = tagBlock.FullSpan then t.FullSpan else t.InnerSpan)
            ...
            let span, motionKind =
                if isOpenLineWise && isCloseLineWise then
                    if openLine.LineNumber + 1 > closeLine.LineNumber - 1 then
                        ...
                        let range =
                            SnapshotLineRangeUtil.CreateForLineNumberRange snapshot (openLine.LineNumber + 1)
    ...
    member x.InnerWord wordKind count contextPoint =
        ...
        let getReverseSpaceAndTabSpaceStart point =
            ...
            if SnapshotPointUtil.IsBlank startPoint then startPoint else SnapshotPointUtil.AddOne startPoint
        ...
        let getSpan (startPoint: SnapshotPoint) point count =
            ...
            | head :: tail ->
                ...
                | Some endPoint ->
                    let startPoint =
                        if head.Start.Position < startPoint.Position
                        then head.Start
                        else startPoint
        ...
        let span =
            if SnapshotPointUtil.IsInsideLineBreak contextPoint && count = 1 then
                ...
                let point =
                    if SnapshotPointUtil.IsBlank contextPoint
                    then getReverseSpaceAndTabSpaceStart contextPoint
                    else contextPoint
        ...
        | Some span ->
            MotionResult.Create
                (span, MotionKind.CharacterWiseInclusive, isForward = true,
            ...
            let rec ep pt counter =
                ...
                let point =
                    ...
                    |> (fun s ->
                        Seq.concat
                            [ s
                              (Seq.singleton (SnapshotUtil.GetEndPoint pt.Snapshot)) ])
                    |> Seq.skipWhile (fun point ->
                        isWhiteSpace pt = isWhiteSpace point && not (lastCount && isEnd point))
                    |> SeqUtil.tryHeadOnly
                if point.IsNone then
                    ...
                    let nextStartPoint =
                        if SnapshotPointUtil.IsInsideLineBreak point.Value
                        then point.Value
    ...
    member x.LineToColumn count =
        x.MotionWithVisualSnapshot(fun x ->
            ...
            let span =
                if isForward
                then SnapshotSpan(x.CaretPoint, targetColumn.StartPoint)
    ...
    member x.CharLeftOnSameLine count =
        ...
        if _vimTextBuffer.UseVirtualSpace && endColumn.IsInVirtualSpace then
            if count < endColumn.VirtualSpaces then
                ...
                MotionResult.Create
                    (span.Span, MotionKind.CharacterWiseExclusive, isForward = false,
                     motionResultFlags = MotionResultFlags.None, caretColumn = CaretColumn.InLastLine columnNumber)
            else
            ...
            if count < endColumn.VirtualSpaces then
                ...
                MotionResult.Create
                    (span.Span, MotionKind.CharacterWiseExclusive, isForward = false,
                     ...
                     caretColumn = CaretColumn.InLastLine startColumn.VirtualColumnNumber)
            else
    ...
    member x.CharRightVirtual count =
        ...
        if endColumn.IsInVirtualSpace then
            MotionResult.Create
                (span.Span, MotionKind.CharacterWiseExclusive, isForward = true,
                 motionResultFlags = MotionResultFlags.None,
                 caretColumn = CaretColumn.InLastLine endColumn.VirtualColumnNumber)
        else
    ...
    member x.CharMotion count withLineWrap =
        if count < 0
        then if withLineWrap then x.CharLeftWithLineWrap -count else x.CharLeftOnSameLine -count
        else if withLineWrap
        then x.CharRightWithLineWrap count
        else x.CharRightOnSameLine count

    /// Count chars left using the h key
    member x.CharLeft count = x.CharMotion -count _globalSettings.IsWhichWrapCharLeft

    /// Count chars right using the l key
    member x.CharRight count = x.CharMotion count _globalSettings.IsWhichWrapCharRight

    /// Count chars left using the backspace key
    member x.SpaceLeft count = x.CharMotion -count _globalSettings.IsWhichWrapSpaceLeft

    /// Count chars right using the space key
    member x.SpaceRight count = x.CharMotion count _globalSettings.IsWhichWrapSpaceRight

    /// Count chars left using the arrow key
    member x.ArrowLeft count = x.CharMotion -count _globalSettings.IsWhichWrapArrowLeft

    /// Count chars right using the arrow key
    member x.ArrowRight count = x.CharMotion count _globalSettings.IsWhichWrapArrowRight
        ...
        x.MotionOptionWithVisualSnapshot(fun x ->
            if x.CaretLine.LineNumber = 0 then
                ...
                MotionResult.CreateLineWise
                    (span, spanBeforeLineWise = characterSpan.Span, isForward = false,
    ...
    member x.LineDown count =
        x.MotionOptionWithVisualSnapshot(fun x ->
            if x.CaretLine.LineNumber = SnapshotUtil.GetLastLineNumber x.CurrentSnapshot then
                ...
                MotionResult.CreateLineWise
                    (span, spanBeforeLineWise = characterSpan.Span, isForward = true,
                     motionResultFlags = MotionResultFlags.MaintainCaretColumn,
                     caretColumn = CaretColumn.InLastLine columnNumber) |> Some)

    /// Get the appropriate maintain caret column flag taking into account 'startofline'
    member x.GetMaintainCaretColumnFlag() =
        if _globalSettings.StartOfLine then MotionResultFlags.None else MotionResultFlags.MaintainCaretColumn
    ...
    member x.LineFromTopOfVisibleWindow countOpt =
        ...
        | NullableUtil.HasValue range ->
            if range.Count = 0 then
                ...
                let minCount =
                    if range.StartLineNumber = 0 then 0 else _globalSettings.ScrollOffset
                ...
                match BufferGraphUtil.MapPointDownToSnapshotStandard _textView.BufferGraph visualLine.Start
        ...
        | NullableUtil.HasValue range ->
            if range.Count = 0 then
                ...
                let minCount =
                    ...
                    if range.LastLineNumber >= lastLineNumber then 0 else _globalSettings.ScrollOffset
    ...
    member x.LineInMiddleOfVisibleWindow() =
        ...
        | NullableUtil.HasValue range ->
            if range.Count = 0 then
                ...
                let middleLine =
                    match BufferGraphUtil.MapPointDownToSnapshotStandard _textView.BufferGraph middleVisualLine.Start
                              x.CurrentSnapshot with
                    | None -> x.CaretLine
    ...
    member x.SectionForwardCore sectionKind context count =
        ...
        let endPoint =
            match context with
            | MotionContext.AfterOperator -> endPoint
    ...
    member x.SectionForwardOrCloseBrace context count = x.SectionForwardCore SectionKind.OnCloseBrace context count

    /// Implements the '[[' motion
    member x.SectionBackwardOrOpenBrace count = x.SectionBackwardCore SectionKind.OnOpenBrace count

    /// Implements the '[]' motion
    member x.SectionBackwardOrCloseBrace count = x.SectionBackwardCore SectionKind.OnCloseBrace count
    ...
    member x.SentenceForward count =
        ...
        let span = SnapshotSpan(x.CaretPoint, endPoint)
        MotionResult.Create
            (span, MotionKind.CharacterWiseExclusive, isForward = true, motionResultFlags = MotionResultFlags.BigDelete)
    ...
    member x.SentenceBackward count =
        ...
        let span = SnapshotSpan(startPoint, x.CaretPoint)
        MotionResult.Create
            (span, MotionKind.CharacterWiseExclusive, isForward = false, motionResultFlags = MotionResultFlags.BigDelete)
    ...
    member x.QuotedString quoteChar =
        ...
        | Some data ->
            let span =
                if not data.TrailingWhiteSpace.IsEmpty
                then SnapshotSpanUtil.Create data.LeadingQuote data.TrailingWhiteSpace.End
    ...
    member x.QuotedStringContentsWithCount quoteChar count =
        ...
        if count > 1 then x.QuotedStringWithoutSpaces quoteChar else QuotedStringContents quoteChar
        ...
        let motionResult =
            ...
            | SearchResult.Found(_, span, _, _) ->
                ...
                | SearchOffsetData.Line _ ->
                    ...
                    motionKind <- MotionKind.LineWise
                    if isForward then caretColumn <- CaretColumn.InLastLine(SnapshotPointUtil.GetLineOffset span.Start)
    ...
    member x.LastSearch isReverse count =
        ...
        if StringUtil.IsNullOrEmpty last.Pattern then
            ...
            let path =
                if isReverse then SearchPath.Reverse last.Path else last.Path
    ...
    member x.NearestMark count exact =
        ...
        let startPoint =
            if exact then caretPoint
            else if isForward then caretLine.End
            else caretLine.Start
        ...
        | None -> None
        | Some(mark, _) ->
            if exact then x.Mark mark else x.MarkLine mark
    ...
    member x.NextPartialWord path count = x.NextWordCore path count false

    /// Motion from the caret to the next occurrence of the word under the caret
    member x.NextWord path count = x.NextWordCore path count true
    ...
    member x.NextWordCore path count isWholeWord =
        ...
        let point =
            ...
            match points
                  |> Seq.filter isWordPoint
                  |> SeqUtil.tryHeadOnly with
            | Some point -> point
            | None ->
                points
                |> Seq.filter SnapshotPointUtil.IsNotBlank
                |> SeqUtil.headOrDefault x.CaretPoint
        ...
        | Some span ->
            ...
            let pattern =
                if isWholeWord then PatternUtil.CreateWholeWord word else word
    ...
    member x.ForceCharacterWise motion motionArgument =
        let convertMotionResult motionResult =
            ...
            | MotionKind.CharacterWiseExclusive ->
                ...
                let span =
                    if isInLineBreakOrEnd span.End then
                        SnapshotSpan(span.Start, 0)
                    else
            ...
            | MotionKind.LineWise ->
                ...
                let span =
                    ...
                    | _, Some p ->
                        if SnapshotLineUtil.IsPhantomLine lineRange.LastLine then
                            SnapshotSpan(span.Start, p)
                        elif SnapshotPointUtil.IsInsideLineBreak p then
                            SnapshotSpan(span.Start, lineRange.LastLine.End)
                        else
                            SnapshotSpan(span.Start, p)
                MotionResult.Create(span, MotionKind.CharacterWiseExclusive, motionResult.IsForward)
    ...
    member x.ForceLineWise motion motionArgument =
        let convertCharacter (motionResult: MotionResult) =
            ...
            let motionResult =
                MotionResult.Create(lineRange.ExtentIncludingLineBreak, MotionKind.LineWise, motionResult.IsForward)
            Some motionResult
    ...
    member x.GetMotion motion (motionArgument: MotionArgument): MotionResult option =
        ...
        let motionResult =
            ...
            | Motion.QuotedStringContents quoteChar ->
                x.QuotedStringContentsWithCount quoteChar motionArgument.CountOrDefault
            ...
            | Motion.SectionForward ->
                x.SectionForward motionArgument.MotionContext motionArgument.CountOrDefault |> Some
            | Motion.SectionForwardOrCloseBrace ->
                x.SectionForwardOrCloseBrace motionArgument.MotionContext motionArgument.CountOrDefault |> Some
            ...
            | Motion.WordForward wordKind ->
                x.WordForward wordKind motionArgument.CountOrDefault motionArgument.MotionContext |> Some
