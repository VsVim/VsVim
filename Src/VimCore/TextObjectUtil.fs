...
type internal TextObjectUtil(_globalSettings: IVimGlobalSettings, _textBuffer: ITextBuffer) =
    ...
    member x.IsEmptyLineWithNoEmptyAbove line =
        if SnapshotLineUtil.IsEmpty line
        then if line.LineNumber = 0 then true else not (x.IsLineAboveEmpty line)
        else false
    ...
    member x.IsBlankOrEmptyLineWithNoBlankOrEmptyAbove line =
        if SnapshotLineUtil.IsBlankOrEmpty line
        then if line.LineNumber = 0 then true else not (x.IsLineAboveBlankOrEmpty line)
        else false
    ...
    member x.IsParagraphStartOnly line =
        ...
        SnapshotPointUtil.IsStartPoint startPoint || x.IsTextMacroMatchLine line _globalSettings.Paragraphs
    ...
    member x.IsSectionStart kind line =
        ...
        let isStandardStart =
            if SnapshotPointUtil.IsStartPoint startPoint
            then true
            elif SnapshotPointUtil.IsChar '\f' startPoint
            then true
            else x.IsTextMacroMatchLine line _globalSettings.Sections

        if isStandardStart then
            true
        else
            match kind with
            | SectionKind.Default -> false
            | SectionKind.OnOpenBrace -> SnapshotPointUtil.IsChar '{' startPoint
            | SectionKind.OnCloseBrace -> SnapshotPointUtil.IsChar '}' startPoint
    ...
    member x.IsSentenceEnd sentenceKind (column: SnapshotColumn) =
        ...
        if column.IsStartOfLine && x.IsSentenceLine line then
            ...
            if (columnBefore.IsLineBreak || columnBefore.IsEndColumn) && x.IsSentenceLine line
            then true
            else x.IsSentenceEndSimple sentenceKind column
    ...
    member x.IsSentenceLastSimple sentenceKind (column: SnapshotColumn) =
        ...
        // logic easier
        let isCharInList list (column: SnapshotColumn) = List.exists (fun (c: char) -> column.IsCharacter(c)) list
        ...
        | SentenceKind.Default ->
            while not pastStart && isCharInList SentenceTrailingChars current do
                if current.IsStartColumn then pastStart <- true else current <- current.Subtract 1

        if pastStart then false else isCharInList SentenceEndChars current

    // Is this a valid white space item to end the sentence
    member x.IsSentenceEndWhiteSpace(column: SnapshotColumn) =
        column.CodePoint.IsWhiteSpace || column.IsLineBreak || column.IsEndColumn
    ...
    member x.IsSentenceStart sentenceKind (column: SnapshotColumn) =
        if x.IsSentenceStartOnly sentenceKind column then
            true
        else if column.IsStartOfLine then
            let line = column.Line
            x.IsParagraphStartOnly line || x.IsSectionStart SectionKind.Default line
        else
            false
    ...
    member x.IsSentenceLine line =
        x.IsTextMacroMatchLine line _globalSettings.Paragraphs || x.IsTextMacroMatchLine line _globalSettings.Sections
    ...
    member x.IsSentenceWhiteSpace sentenceKind (column: SnapshotColumn) =
        if column.IsStartOfLine && x.IsEmptyLineWithNoEmptyAbove column.Line then
            ...
            let mutable current = column
            while not (x.IsSentenceEnd sentenceKind current) && x.IsSentenceEndWhiteSpace current
                  && not current.IsStartColumn do
                current <- current.Subtract 1
    ...
    member x.GetSectionRanges sectionKind path point =
        ...
        | SearchPath.Backward ->
            ...
            let point =
                let line = SnapshotPointUtil.GetContainingLine point
                if x.IsSectionStart sectionKind line then line.Start else point
    ...
    member x.GetSections sectionKind path point =
        x.GetSectionRanges sectionKind path point |> Seq.map (fun range -> range.ExtentIncludingLineBreak)
    ...
    member x.GetTextObjectsCore (column: SnapshotColumn) path (isStartColumn: SnapshotColumn -> bool)
           (getSpanFromStartColumn: SnapshotColumn -> SnapshotSpan) =
        ...
        let getSpanFromStartColumn (column: SnapshotColumn) =
            if column.IsEndColumn then SnapshotSpan(column.StartPoint, 0) else getSpanFromStartColumn column
        ...
        | SearchPath.Forward, _ ->
            ...
            let startColumn =
                ...
                if column.StartPosition >= span.End.Position then
                    ...
                    if forwardColumn.StartPosition > column.StartPosition
                    then startColumn
                    else forwardColumn
                else
                    startColumn
