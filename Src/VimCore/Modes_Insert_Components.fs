...
type internal TextChangeTracker(_vimTextBuffer: IVimTextBuffer, _textView: ITextView, _operations: ICommonOperations) as this =
    ...
    do
        ...
        _textView.Closed |> Event.add (fun _ -> _bag.DisposeAll())
    ...
    /// Clear out the current change without completing it
    member x.ClearChange() = _currentTextChange <- None
    ...
    member x.IsEffectiveChangeInsert =
        ...
        | Some data -> data.LeftDeletions = 0 && data.RightDeletions = 0
        | None -> false
    ...
    member x.EffectiveChange =
        ...
        | Some data when data.Snapshot = snapshot ->
            SnapshotSpan(snapshot, data.LeftEdge, data.RightEdge - data.LeftEdge) |> Some
        | _ -> None

    /// Stop tracking the effective change
    member x.StopTrackingEffectiveChange() = _effectiveChangeData <- None
    ...
    member x.ConvertBufferChange (beforeSnapshot: ITextSnapshot) (change: ITextChange) =
        ...
        let convert() =
            ...
            let getDelete() =
                let caretPoint = TextViewUtil.GetCaretPoint _textView
                let isCaretAtStart = caretPoint.Snapshot = beforeSnapshot && caretPoint.Position = change.OldPosition
        ...
        if change.OldText.Length > 0 && StringUtil.IsBlanks change.NewText && StringUtil.IsBlanks change.OldText then
            let spacesToColumn =
                SnapshotUtil.GetPoint beforeSnapshot change.OldSpan.Start |> _operations.GetSpacesToPoint
            let oldText = _operations.NormalizeBlanks change.OldText spacesToColumn
            let newText = _operations.NormalizeBlanks change.NewText spacesToColumn
    ...
    member x.OnTextChanged(args: TextContentChangedEventArgs) =
        ...
        if x.TrackCurrentChange then x.UpdateCurrentChange args

        if _effectiveChangeData.IsSome then x.UpdateEffectiveChange args
    ...
    member x.UpdateCurrentChange(args: TextContentChangedEventArgs) =
        ...
        if args.Changes.Count = 1 then
            ...
            | Some(oldTextChange, oldBufferChange) ->
                x.MergeChange oldTextChange oldBufferChange newTextChange newBufferChange
        else
            x.CompleteChange()
    ...
    member x.UpdateMarks(args: TextContentChangedEventArgs) =
        ...
        if args.Changes.Count = 1 then
            ...
            let position =
                if change.Delta > 0 && change.NewEnd > 0 then change.NewEnd - 1 else change.NewPosition
    ...
    member x.UpdateEffectiveChange(args: TextContentChangedEventArgs) =
        ...
        for i = 0 to args.Changes.Count - 1 do
            ...
            VimTrace.TraceInfo
                ("OnTextChange: old = '{0}', new = '{1}'", StringUtil.GetDisplayString(change.OldText),
                 StringUtil.GetDisplayString(change.NewText))
        ...
        | Some data when data.Snapshot = args.Before ->
            ...
            for i = args.Changes.Count - 1 downto 0 do
                ...
                if args.Before = caretPosition.BufferPosition.Snapshot && virtualSpaces > 0
                   && change.OldSpan.Start = leftEdge && change.OldSpan.End = leftEdge
                   && change.NewText.Length >= virtualSpaces
                   && change.NewText.Substring(0, virtualSpaces) |> StringUtil.IsBlanks then
