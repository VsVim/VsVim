...
type internal DisabledMode(_vimBufferData: IVimBufferData) =
    ...
    member x.HelpString =
        ...
        if VimKeyModifiers.None = keyInput.KeyModifiers then
            ...
            sprintf "Vim Disabled. Type %s+%s to re-enable" (keyInput.Key.ToString())
                (keyInput.KeyModifiers.ToString())

    member x.Process(keyInputData: KeyInputData) =
        if keyInputData.KeyInput = _globalSettings.DisableAllCommand
        then ProcessResult.OfModeKind ModeKind.Normal
        else ProcessResult.NotHandled
