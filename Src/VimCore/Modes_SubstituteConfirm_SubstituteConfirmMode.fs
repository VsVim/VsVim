...
type internal SubstituteConfirmMode(_vimBufferData: IVimBufferData, _operations: ICommonOperations) as this =
    ...
    member x.CanProcess(keyInput: KeyInput) = KeyInputUtil.IsCore keyInput && not keyInput.IsMouseKey
    ...
    member x.MoveToNext(data: ConfirmData) =
        ...
        let point =
            if data.CurrentMatch.Snapshot = _textBuffer.CurrentSnapshot then
                ...
                let point =
                    data.CurrentMatch.Snapshot.CreateTrackingPoint
                        ((data.CurrentMatch.End.Position), PointTrackingMode.Positive)
        ...
        let rec doSearch point =
            ...
            if line.LineNumber > data.LastLineNumber || SnapshotPointUtil.IsEndPoint point then
                ...
                | Some(span, _) ->
                    ...
                    ModeSwitch.NoSwitch
                | None -> doSearch line.EndIncludingLineBreak
    ...
    member x.DoSubstituteAll data =
        ...
        let lineSpans =
            ...
            let rest =
                if line.LineNumber = data.LastLineNumber || line.LineNumber = line.Snapshot.LineCount - 1 then
                    ...
                    let range =
                        SnapshotLineRangeUtil.CreateForLineNumberRange line.Snapshot (line.LineNumber + 1)
                            data.LastLineNumber
        ...
        let replaceData =
            ...
            then { replaceData with Count = VimRegexReplaceCount.All }
            else replaceData
    ...
    interface ISubstituteConfirmMode with
        ...
        member x.Process keyInputData =
            ...
            match _confirmData with
            | None -> ProcessResult.OfModeKind ModeKind.Normal
            | Some data ->
            ...
            x.ConfirmData <-
                ...
                | ModeArgument.Substitute(span, range, data) ->
                    ...
                    | Some regex ->
                        ...
                        let data =
                            { Regex = regex
                              SubstituteText = data.Substitute
                              CurrentMatch = span
                              LastLineNumber = range.LastLineNumber
                              IsReplaceAll = isReplaceAll }
                        Some data
                | _ -> None

        member x.OnLeave() = x.ConfirmData <- None
