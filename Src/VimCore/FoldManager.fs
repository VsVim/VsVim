...
[<Sealed>]
type internal FoldData(_textBuffer: ITextBuffer) =
    ...
    member x.DeleteAllFolds(span: SnapshotSpan) =
        _folds <-
            ...
            |> Seq.map
                (fun span -> _textBuffer.CurrentSnapshot.CreateTrackingSpan(span.Span, SpanTrackingMode.EdgeInclusive))
            |> List.ofSeq
        _foldsUpdated.Trigger x
...
type internal FoldManager(_textView: ITextView, _foldData: IFoldData, _statusUtil: IStatusUtil, _outliningManager: IOutliningManager option) =
    ...
    member x.DeleteFold point =
        if not (_foldData.DeleteFold point) then _statusUtil.OnError Resources.Common_NoFoldFound

    member x.DeleteAllFolds span = _foldData.DeleteAllFolds span
    ...
    member x.ToggleFold point count =
        x.DoWithOutliningManager(fun outliningManager ->
            ...
            let currentRegions =
                outliningManager.GetAllRegions(x.GetSpanForRegions(point))
                |> List.ofSeq
                |> List.rev
                |> Seq.truncate count

            if (currentRegions.FirstOrDefault().IsCollapsed)
            then x.OpenFold point count
            else x.CloseFold point count)
    ...
    member x.ToggleAllFolds(span: SnapshotSpan) =
        x.DoWithOutliningManager(fun outliningManager ->
            ...
            if not currentRegions.IsEmpty then
                if (currentRegions.First().IsCollapsed) then x.OpenAllFolds span else x.CloseAllFolds span)
...
type FoldManagerFactory [<ImportingConstructor>] (_statusUtilFactory: IStatusUtilFactory, _outliningManagerService: IOutliningManagerService) =
            ...
            (_managerKey,
             (fun unused ->
                 let outliningManager =
                     ...
                     if outliningManager = null then None else Some outliningManager
