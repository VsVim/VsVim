...
[<Sealed>]
[<Class>]
type InterpreterStatusUtil(_statusUtil: IStatusUtil) =
...
[<Sealed>]
[<Class>]
type VariableValueUtil(_statusUtil: IStatusUtil) =
...
[<Sealed>]
[<Class>]
type BuiltinFunctionCaller(_variableMap: Dictionary<string, VariableValue>) =
...
[<Sealed>]
[<Class>]
type VimScriptFunctionCaller(_builtinCaller: BuiltinFunctionCaller, _statusUtil: IStatusUtil) =
    ...
    member x.Call (name: VariableName) (args: VariableValue list) =
        ...
        | "localtime" ->
            if args.Length = 0
            then _builtinCaller.Call BuiltinFunctionCall.Localtime
            else tooManyArgs()
        | "nr2char" ->
            match args.Length with
            | 0 -> notEnoughArgs()
...
[<Sealed>]
[<Class>]
type ExpressionInterpreter(_statusUtil: IStatusUtil, _localSettings: IVimSettings, _windowSettings: IVimSettings, _variableMap: Dictionary<string, VariableValue>, _registerMap: IRegisterMap) =
    ...
    member x.GetExpressionAsNumber expr = x.RunExpression expr |> _getValue.ConvertToNumber

    /// Get the specified expression as a number
    member x.GetExpressionAsString expr = x.RunExpression expr |> _getValue.ConvertToString
    ...
    member x.GetValueOfVariable name =
        ...
        if found then value else VariableValue.Error

    member x.GetValueOfRegister name = (_registerMap.GetRegister name).StringValue |> VariableValue.String

    /// Get the value of the specified expression
    member x.RunExpression(expr: Expression): VariableValue =
        let runExpression expressions =
            [ for expr in expressions -> x.RunExpression expr ]
        match expr with
    ...
    member x.RunBinaryExpression binaryKind (leftExpr: Expression) (rightExpr: Expression) =
        ...
        let runNumericBinary (leftValue: VariableValue) (rightValue: VariableValue) (binaryKind: BinaryKind) =
            if leftValue.VariableType = VariableType.List || rightValue.VariableType = VariableType.List then
                ...
                | Some left, Some right, BinaryKind.GreaterThan ->
                    left > right
                    |> System.Convert.ToInt32
                    |> VariableValue.Number
                | Some left, Some right, BinaryKind.LessThan ->
                    left < right
                    |> System.Convert.ToInt32
                    |> VariableValue.Number
                | _ -> VariableValue.Error
        ...
        | BinaryKind.LessThan -> runNumericBinary leftValue rightValue binaryKind
        | BinaryKind.Equal
        | BinaryKind.NotEqual -> runStringEquality leftValue rightValue binaryKind

[<Sealed>]
[<Class>]
type VimInterpreter(_vimBuffer: IVimBuffer, _commonOperations: ICommonOperations, _foldManager: IFoldManager, _fileSystem: IFileSystem, _bufferTrackingService: IBufferTrackingService) =
    ...
    let _exprInterpreter =
        ExpressionInterpreter(_statusUtil, _localSettings, _windowSettings, _variableMap, _registerMap)
    ...
    member x.KeyMaps =
        [ _vim.GlobalKeyMap :> IVimKeyMap
          _vimTextBuffer.LocalKeyMap :> IVimKeyMap ]
    ...
    member x.GetLineAndVimLineNumberCore lineSpecifier currentLine =
        ...
        let getAdjustment adjustment (line: ITextSnapshotLine) =
            ...
            SnapshotUtil.TryGetLine x.CurrentSnapshot number |> Option.map (fun line -> line, vimLine)
        ...
        | LineSpecifier.LastLine ->
            SnapshotUtil.GetLastNormalizedLine x.CurrentSnapshot
            |> getLineAndNumber
            |> Some
        ...
        | LineSpecifier.Number number ->
            ...
            SnapshotUtil.TryGetLine x.CurrentSnapshot tssNumber |> Option.map (fun line -> line, number)
        ...
        | LineSpecifier.NextLineWithPattern pattern ->
            let pattern =
                if pattern = "" then _vim.VimData.LastSearchData.Pattern else pattern
            x.GetMatchingLine pattern SearchPath.Forward currentLine |> Option.map getLineAndNumber
        ...
        | LineSpecifier.PreviousLineWithPattern pattern ->
            let pattern =
                if pattern = "" then _vim.VimData.LastSearchData.Pattern else pattern
            x.GetMatchingLine pattern SearchPath.Backward currentLine |> Option.map getLineAndNumber
    ...
    member x.GetMatchingLine pattern path currentLine =
        if StringUtil.IsNullOrEmpty pattern then
            ...
            let startPoint =
                match path with
                | SearchPath.Forward -> currentLine |> SnapshotLineUtil.GetEnd
                | SearchPath.Backward -> currentLine |> SnapshotLineUtil.GetStart
    ...
    // Get a tuple of the ITextSnapshotLine specified by the given LineSpecifier and the
    // corresponding vim line number
    member x.GetLineAndVimLineNumber lineSpecifier = x.GetLineAndVimLineNumberCore lineSpecifier x.CaretLine

    /// Get the ITextSnapshotLine specified by the given LineSpecifier
    member x.GetLineCore lineSpecifier currentLine =
        x.GetLineAndVimLineNumberCore lineSpecifier currentLine |> Option.map (fun (line, vimLine) -> line)

    /// Get the ITextSnapshotLine specified by the given LineSpecifier
    member x.GetLine lineSpecifier = x.GetLineCore lineSpecifier x.CaretLine
    ...
    member x.GetLineRangeOrDefault lineRange defaultLineRange =
        ...
        | LineRangeSpecifier.EntireBuffer -> SnapshotLineRangeUtil.CreateForSnapshot x.CurrentSnapshot |> Some
        ...
        | LineRangeSpecifier.Range(leftLineSpecifier, rightLineSpecifier, adjustCaret) ->
            ...
            | Some leftLine, Some rightLine ->
                ...
                if leftLine.Snapshot = rightLine.Snapshot then
                    let result = SnapshotLineRangeUtil.CreateForLineRange leftLine rightLine |> Some
    ...
    member x.GetLineRange lineRange = x.GetLineRangeOrDefault lineRange DefaultLineRange.None
    ...
    member x.GetPointAfterOrDefault lineRange defaultLineRange =
        ...
        | _ ->
            match x.GetLineRangeOrDefault lineRange defaultLineRange with
            | Some lineRange -> lineRange.EndIncludingLineBreak |> Some
            | None -> None

    /// Get the point after the specified range or the the default
    member x.GetPointAfter lineRange = x.GetPointAfterOrDefault lineRange DefaultLineRange.None
    ...
    member x.RunAbbreviate lhs rhs allowRemap modeList isLocal =
        ...
        | Some _ ->
            let map =
                if isLocal
                then _vimTextBuffer.LocalAbbreviationMap :> IVimAbbreviationMap
        ...
        | Some keyInputSet ->
            ...
            for mode in modeList do
                if not (map.RemoveAbbreviation(keyInputSet, mode)) then
                    ...
                    let all =
                        map.GetAbbreviations mode
                        |> Seq.filter (fun x -> x.Replacement = keyInputSet)
                        |> List.ofSeq
                    if all.IsEmpty then
                        succeeded <- false
                    else
            ...
            if not succeeded then _statusUtil.OnError Resources.Parser_InvalidArgument
    ...
    member x.RunCall(callInfo: CallInfo) = _statusUtil.OnError(Resources.Interpreter_CallNotSupported callInfo.Name)
    ...
    member x.RunChangeDirectory symbolicPath =
        ...
        | _ ->
            ...
            let directoryPath =
                if not (Path.IsPathRooted directoryPath)
                then Path.GetFullPath(Path.Combine(_vimData.CurrentDirectory, directoryPath))
                else directoryPath
    ...
    member x.RunCopyOrMoveTo sourceLineRange destLineRange count transactionName editOperation =
        ...
        x.RunWithLineRangeOrDefault sourceLineRange DefaultLineRange.CurrentLine (fun sourceLineRange ->
            ...
            | Some(destLine, destLineNum) ->
                ...
                // operation
                if _textView.TextSnapshot = destPoint.Snapshot then
                    _undoRedoOperations.EditWithUndoTransaction transactionName _textView
                        (fun () -> editOperation sourceLineRange destPoint text)
                else
    ...
    member x.RunMoveTo sourceLineRange destLineRange count =
        x.RunCopyOrMoveTo sourceLineRange destLineRange count "MoveTo" (fun sourceLineRange destPoint text ->
            ...
            if sourceLineRange.Start.Snapshot = destPoint.Snapshot
            then edit.Delete(sourceLineRange.ExtentIncludingLineBreak.Span) |> ignore
            else sourceTextBuffer.Delete(sourceLineRange.ExtentIncludingLineBreak.Span) |> ignore
            edit.Apply() |> ignore
    ...
    member x.RunCompose lineCommand1 lineCommand2 =
        use transaction =
            _undoRedoOperations.CreateLinkedUndoTransactionWithFlags "Compose Command"
    ...
    member x.RunClearKeyMap keyRemapModes mapArgumentList =
        ...
        if not (List.isEmpty mapArgumentList)
        then _statusUtil.OnError(Resources.Interpreter_OptionNotSupported "map special arguments")
        else keyRemapModes |> Seq.iter keyMap.ClearKeyMappings

    /// Run the close command
    member x.RunClose hasBang =
        if hasBang
        then _vimHost.Close _textView
        else _commonOperations.CloseWindowUnlessDirty()

    /// Run the delete command.  Delete the specified range of text and set it to
    /// the given Register
    member x.RunDelete lineRange registerName =
        x.RunWithLineRangeOrDefault lineRange DefaultLineRange.CurrentLine
            (fun lineRange -> _commonOperations.DeleteLines lineRange.StartLine lineRange.Count registerName)

    member x.RunDeleteMarkCore mark =
        let markMap = _vimBufferData.Vim.MarkMap
        markMap.DeleteMark mark _vimBufferData |> ignore

    member x.RunDeleteMarks marks = marks |> Seq.iter x.RunDeleteMarkCore
    ...
    member x.RunFunction(func: Function) = _statusUtil.OnError Resources.Interpreter_FunctionNotSupported
    ...
    member x.RunDigraphs digraphList =
        ...
        let getChars char1 char2 = string (char1) + string (char2)
        ...
        if List.isEmpty digraphList then
            let columns = 4
            digraphMap.Mappings
            |> Seq.map (fun (char1, char2, code) -> (getChars char1 char2, getDisplay code, code))
            |> Seq.sortBy (fun (_, _, code) -> code)
            |> Seq.mapi (fun index (digraph, text, code) -> (index + 1, sprintf "%2s %-2s %5d  " digraph text code))
            |> Seq.map
                (fun (index, output) -> output + (if index % columns = 0 then System.Environment.NewLine else ""))
            |> String.concat ""
    ...
    member x.RunDisplayAbbreviation modeList (lhs: string option) =
        ...
        let appendMap (map: IVimAbbreviationMap) isLocalChar =
            ...
            let add mode str =
                if Seq.contains mode modeList then list.Add str
            ...
            let getDisplayString key (abbreviations: Abbreviation seq) =
                ...
                | [ a1; a2 ] when a1.Replacement = a2.Replacement ->
                    list.Add(sprintf "!  %O\t%c%O" key isLocalChar a1.Replacement)
                | [ a1; a2 ] ->
    ...
    member x.RunDisplayKeyMap keyRemapModes prefixFilter =
        ...
        let nvoGroup = [ KeyRemapMode.Normal; KeyRemapMode.Visual; KeyRemapMode.Select; KeyRemapMode.OperatorPending ]
        let vGroup = [ KeyRemapMode.Visual; KeyRemapMode.Select ]
        let bangGroup = [ KeyRemapMode.Insert; KeyRemapMode.Command ]
        ...
        let separateOutGroup (modeGroup: KeyRemapMode list) (modes: KeyRemapMode list) =
            let matches = modes |> List.filter (fun mode -> modeGroup |> Seq.contains mode)
            ...
            let nonMatches =
                ...
                |> List.filter (fun mode ->
                    ...
                    |> Seq.contains mode
                    |> not)
            if matches.Length = modeGroup.Length then [ modeGroup ], nonMatches else List.Empty, modes
        ...
        let filterByPrefix sequence =
            ...
            | Some prefixNotation ->
                ...
                | Some prefix -> sequence |> Seq.filter (fun (_, lhs, _) -> startsWith lhs prefix)
                | None -> sequence
            | None -> sequence
        ...
        let getKeyInputSetLine (keyInputSet: KeyInputSet) = KeyNotationUtil.KeyInputSetToString keyInputSet
    ...
    member x.RunDisplayLet(names: VariableName list) =
        ...
        seq {
            for name in names do
                ...
                yield if found
                      then sprintf "%s %O" name.Name value
                      else Resources.Interpreter_UndefinedVariable name.Name
        }
        |> _statusUtil.OnStatusLong
    ...
    member x.RunDisplayMarks(marks: Mark list) =
        ...
        let getMark (mark: Mark) =
            ...
            | Some markInfo -> (markInfo.Ident, markInfo.Name, markInfo.Line, markInfo.Column) |> Some
            | None -> None
    ...
    member x.RunExecute expression =
        ...
        let execute str = parser.ParseLineCommand str |> x.RunLineCommand
    ...
    /// Get the value of the specified expression
    member x.RunExpression expr = _exprInterpreter.RunExpression expr
    ...
    member x.EvaluateExpression(text: string) =
        ...
        | ParseResult.Succeeded expression -> _exprInterpreter.RunExpression expression |> EvaluateResult.Succeeded
        | ParseResult.Failed message -> EvaluateResult.Failed message
    ...
    member x.RunFold lineRange =
        ...
        x.RunWithLineRangeOrDefault lineRange DefaultLineRange.CurrentLine (fun lineRange ->
            if lineRange.Count > 1 then _foldManager.CreateFold lineRange)
    ...
    member x.RunNormal (lineRange: LineRangeSpecifier) input =
        ...
        try
            let rec inner list =
                ...
                | keyInput :: tail ->
                    ...
                    if not (transactionMap.ContainsKey(buffer)) then
                        let transaction =
                            _undoRedoOperations.CreateLinkedUndoTransactionWithFlags "Normal Command"
                                LinkedUndoTransactionFlags.CanBeEmpty
            ...
            let revertModes() =
                modeSwitchMap.Values |> Seq.iter (fun buffer -> buffer.SwitchPreviousMode() |> ignore)
                modeSwitchMap.Clear()
            ...
            | _ ->
                x.RunWithLineRange lineRange (fun lineRange ->
                    ...
                    lineNumbers
                    |> List.iter (fun trackingLineColumn ->
            ...
            transactionMap.Values |> Seq.iter (fun transaction -> transaction.Dispose())
    ...
    member x.RunGlobal lineRange pattern matchPattern lineCommand =
        ...
        x.RunWithLineRangeOrDefault lineRange DefaultLineRange.EntireBuffer (fun lineRange ->
            ...
            | Some regex ->
                ...
                // global undo units
                use transaction =
                    _undoRedoOperations.CreateLinkedUndoTransactionWithFlags "Global Command"
                        LinkedUndoTransactionFlags.CanBeEmpty
                try
    ...
    /// Go to the first tab
    member x.RunGoToFirstTab() = _commonOperations.GoToTab 0

    /// Go to the last tab
    member x.RunGoToLastTab() = _commonOperations.GoToTab _vimHost.TabCount
    ...
    member x.RunVimGrep count hasBang pattern flags filePattern =
        ...
        | Some regex ->
            ...
            let matchCase =
                ...
                | CaseSpecifier.IgnoreCase -> false
                | CaseSpecifier.OrdinalCase -> true
                | CaseSpecifier.None -> not _globalSettings.IgnoreCase
    ...
    member x.RunVimHelp(subject: string) =
        ...
        let extractFolderVersion =
            fun pathName ->
                let folder = System.IO.Path.GetFileName(pathName)
                let m =
                    System.Text.RegularExpressions.Regex.Match
                        (folder, "^vim(\d+)", System.Text.RegularExpressions.RegexOptions.IgnoreCase)
                match m with
        ...
        let onStatusFocusedWindow message =
            fun (commonOperations: ICommonOperations) -> commonOperations.OnStatusFitToWindow message
    ...
    member x.RunIf(conditionalBlockList: ConditionalBlock list) =
        ...
        | Some conditionalBlock ->
            conditionalBlock.LineCommands |> Seq.iter (fun lineCommand -> x.RunLineCommand lineCommand |> ignore)

    /// Join the lines in the specified range
    member x.RunJoin lineRange joinKind =
        x.RunWithLineRangeOrDefault lineRange DefaultLineRange.CurrentLine
            (fun lineRange -> _commonOperations.Join lineRange joinKind)

    /// Jump to the last line of the specified line range
    member x.RunJumpToLastLine lineRange =
        x.RunWithLooseLineRangeOrDefault lineRange DefaultLineRange.CurrentLine
            (fun lineRange -> x.MoveLinewiseToPoint lineRange.LastLine.Start)
    ...
    member x.RunMake hasBang arguments = _vimHost.Make (hasBang) arguments
    ...
    member x.RunMapKeys leftKeyNotation rightKeyNotation keyRemapModes allowRemap mapArgumentList =
        ...
        | Some lhs, Some rhs ->
            ...
            keyRemapModes |> Seq.iter (fun keyRemapMode -> keyMap.AddKeyMapping(lhs, rhs, allowRemap, keyRemapMode))
        | _ -> _statusUtil.OnError(Resources.Interpreter_UnableToMapKeys leftKeyNotation rightKeyNotation)

    /// Run the 'nohlsearch' command.  Temporarily disables highlighitng in the buffer
    member x.RunNoHighlightSearch() = _vimData.SuspendDisplayPattern()

    /// Report a parse error that has already been line number annotated
    member x.RunParseError msg = _vimBufferData.StatusUtil.OnError msg
    ...
    member x.RunDisplayLines lineRange lineCommandFlags =
        ...
        x.RunWithLineRangeOrDefault lineRange DefaultLineRange.CurrentLine (fun lineRange ->
            ...
            let formatLine (line: ITextSnapshotLine) =
                if addLineNumber || _localSettings.Number then formatLineNumberAndLine line else printOrListLine line
    ...
    member x.RunPrintCurrentDirectory() = _statusUtil.OnStatus x.CurrentDirectory
    ...
    member x.RunPut lineRange registerName putAfter =
        ...
        x.RunWithLineRangeOrDefault lineRange DefaultLineRange.CurrentLine (fun lineRange ->
            ...
            _undoRedoOperations.EditWithUndoTransaction "PutLine" _textView (fun () ->
                ...
                let line =
                    if putAfter then lineRange.LastLine else lineRange.StartLine

                let point =
                    if putAfter then line.EndIncludingLineBreak else line.Start
                ...
                let line =
                    let number =
                        if putAfter then line.LineNumber + 1 else line.LineNumber
    ...
    member x.RunOpenListWindow listKind = _vimHost.OpenListWindow listKind
    ...
    member x.RunNavigateToListItem listKind navigationKind argument hasBang =
        ...
        let onNavigateDone (listItem: ListItem) (commonOperations: ICommonOperations) =
            ...
            sprintf "(%d of %d): %s" itemNumber listLength message |> commonOperations.OnStatusFitToWindow
    ...
    member x.RunQuit hasBang = x.RunClose hasBang
    ...
    member x.RunReadCommand lineRange command = x.ExecuteCommand lineRange command true
    ...
    member x.RunReadFile lineRange fileOptionList filePath =
        ...
        x.RunWithPointAfterOrDefault lineRange DefaultLineRange.CurrentLine (fun point ->
            if not (List.isEmpty fileOptionList) then
                ...
                | None -> _statusUtil.OnError(Resources.Interpreter_CantOpenFile filePath)
                | Some lines -> x.RunReadCore point lines)

    /// Run a single redo operation
    member x.RunRedo() = _commonOperations.Redo 1
    ...
    member x.RunSearch lineRange path pattern =
        x.RunWithLineRangeOrDefault lineRange DefaultLineRange.CurrentLine (fun lineRange ->
            let pattern =
                if StringUtil.IsNullOrEmpty pattern then _vimData.LastSearchData.Pattern else pattern

            // The search start after the end of the specified line range or
            // before its beginning.
            let startPoint =
                match path with
                | SearchPath.Forward -> lineRange.End
                | SearchPath.Backward -> lineRange.Start

            let searchData = SearchData(pattern, path, _globalSettings.WrapScan)
            let result =
                _searchService.FindNextPattern startPoint searchData _vimTextBuffer.WordUtil.SnapshotWordNavigator 1
    ...
    member x.RunSet setArguments =
        ...
        let getSettingDisplay (setting: Setting) =
            ...
            | SettingValue.Toggle b ->
                if b then setting.Name else sprintf "no%s" setting.Name
            | SettingValue.String s -> sprintf "%s=\"%s\"" setting.Name s
            | SettingValue.Number n -> sprintf "%s=%d" setting.Name n
        ...
        let invertSetting name =
            ...
            withSetting name msg (fun setting container ->
                ...
                | _ ->
                    msg
                    |> Resources.CommandMode_InvalidArgument
                    |> _statusUtil.OnError)
    ...
    /// Run the ':!' command
    member x.RunShellCommand lineRange command = x.ExecuteCommand lineRange command false
    ...
    member x.ExecuteCommand (lineRange: LineRangeSpecifier) (command: string) (isReadCommand: bool) =
        ...
        let doRun (command: string) =
            ...
            let command =
                if _globalSettings.ShellFlag.Length > 0
                then sprintf "%s %s" _globalSettings.ShellFlag command
                else command
            ...
            else if lineRange = LineRangeSpecifier.None then
                let results = _vimHost.RunCommand workingDirectory shell command StringUtil.Empty
                let status = results.Output + results.Error
                let status = EditUtil.RemoveEndingNewLine status
                _statusUtil.OnStatus status
            else
                x.RunWithLineRangeOrDefault lineRange DefaultLineRange.None
                    (fun lineRange -> _commonOperations.FilterLines lineRange command)
        ...
        let rec inner index afterBackslash =
            if index >= command.Length then
                ...
                if current = '\\' && (index + 1) < command.Length then
                    ...
                    | None -> _statusUtil.OnError Resources.Common_NoPreviousShellCommand
    ...
    member x.RunShiftLeft lineRange count =
        x.RunWithLineRangeOrDefault lineRange DefaultLineRange.CurrentLine
            (fun lineRange -> _commonOperations.ShiftLineRangeLeft lineRange count)

    /// Shift the given line range to the right
    member x.RunShiftRight lineRange count =
        x.RunWithLineRangeOrDefault lineRange DefaultLineRange.CurrentLine
            (fun lineRange -> _commonOperations.ShiftLineRangeRight lineRange count)

    /// Run the :sort command
    member x.RunSort lineRange reverseOrder flags pattern =
        x.RunWithLineRangeOrDefault lineRange DefaultLineRange.EntireBuffer
            (fun lineRange -> _commonOperations.SortLines lineRange reverseOrder flags pattern)
        ...
        if hasBang then
            ...
            | Some lines ->
                ...
                if errorList.Count <> 0 then
                    seq {
                        let message = Resources.Interpreter_ErrorsSourcing filePath |> _statusUtil.NotateMessage
                        yield message
                        yield! errorList
                    }
    ...
    member x.RunSplit behavior fileOptions commandOption =
        ...
        if SplitArgumentsAreValid fileOptions commandOption
        then behavior _textView
        else ()
    ...
    member x.RunOnly() = _vimHost.CloseAllOtherWindows _textView

    member x.RunTabOnly() = _vimHost.CloseAllOtherTabs _textView

    /// Run the undo command
    member x.RunUndo() = _commonOperations.Undo 1
            ...
            | Some key ->
                ...
                for keyRemapMode in keyRemapModes do
                    ...
                    if not (keyMap.RemoveKeyMapping(key, keyRemapMode)) then
                        let all =
                            keyMap.GetKeyMappings(keyRemapMode)
                            |> Seq.filter (fun x -> x.Right = key)
                            |> List.ofSeq
                        if all.IsEmpty then
                            succeeded <- false
                        else
                ...
                if not succeeded then _statusUtil.OnError Resources.CommandMode_NoSuchMapping
            | None -> _statusUtil.OnError Resources.Parser_InvalidArgument
    ...
    member x.RunWrite lineRange hasBang fileOptionList filePath =
        x.RunWithLineRangeOrDefault lineRange DefaultLineRange.EntireBuffer (fun lineRange ->
            ...
            if not (List.isEmpty fileOptionList) then
                ...
                | None ->
                    if not hasBang && _vimHost.IsReadOnly _textBuffer
                    then _statusUtil.OnError Resources.Interpreter_ReadOnlyOptionIsSet
                    else _vimHost.Save _textBuffer |> ignore)
    ...
    member x.RunWriteAll hasBang andQuit =
        ...
        if andQuit then
            if succeeded
            then _vimHost.Quit()
            else _statusUtil.OnError Resources.Common_NoWriteSinceLastChange
    ...
    member x.RunLineCommand lineCommand =
        ...
        | LineCommand.Abbreviate(lhs, rhs, allowRemap, modeList, isLocal) ->
            x.RunAbbreviate lhs rhs allowRemap modeList isLocal
        ...
        | LineCommand.CopyTo(sourceLineRange, destLineRange, count) ->
            x.RunCopyTo sourceLineRange destLineRange count
        ...
        | LineCommand.DisplayKeyMap(keyRemapModes, keyNotationOption) ->
        ...
        | LineCommand.Edit(hasBang, fileOptions, commandOption, filePath) ->
            x.RunEdit hasBang fileOptions commandOption filePath
        ...
        | LineCommand.Global(lineRange, pattern, matchPattern, lineCommand) ->
            x.RunGlobal lineRange pattern matchPattern lineCommand
        ...
        | LineCommand.HorizontalSplit(lineRange, fileOptions, commandOptions) ->
            x.RunSplit _vimHost.SplitViewHorizontally fileOptions commandOptions
        ...
        | LineCommand.MoveTo(sourceLineRange, destLineRange, count) ->
            x.RunMoveTo sourceLineRange destLineRange count
        | LineCommand.NavigateToListItem(listKind, navigationKind, argument, hasBang) ->
            x.RunNavigateToListItem listKind navigationKind argument hasBang
        ...
        | LineCommand.QuitWithWrite(lineRange, hasBang, fileOptions, filePath) ->
            x.RunQuitWithWrite lineRange hasBang fileOptions filePath
        ...
        | LineCommand.ReadFile(lineRange, fileOptionList, filePath) ->
            x.RunReadFile lineRange fileOptionList filePath
        ...
        | LineCommand.Substitute(lineRange, pattern, replace, flags) ->
            x.RunSubstitute lineRange pattern replace flags
        | LineCommand.SubstituteRepeat(lineRange, substituteFlags) ->
            x.RunSubstituteRepeatLast lineRange substituteFlags
        ...
        | LineCommand.UnmapKeys(keyNotation, keyRemapModes, mapArgumentList) ->
            x.RunUnmapKeys keyNotation keyRemapModes mapArgumentList
        | LineCommand.Version -> x.RunVersion()
        | LineCommand.VerticalSplit(lineRange, fileOptions, commandOptions) ->
            x.RunSplit _vimHost.SplitViewVertically fileOptions commandOptions
        | LineCommand.VimGrep(count, hasBang, pattern, flags, filePattern) ->
            x.RunVimGrep count hasBang pattern flags filePattern
        | LineCommand.VimHelp subject -> x.RunVimHelp subject
        | LineCommand.Write(lineRange, hasBang, fileOptionList, filePath) ->
            x.RunWrite lineRange hasBang fileOptionList filePath
    ...
    member x.RunWithLineRangeOrDefault (lineRangeSpecifier: LineRangeSpecifier) defaultLineRange
           (func: SnapshotLineRange -> unit) =
    ...
    member x.RunWithLooseLineRangeOrDefault (lineRangeSpecifier: LineRangeSpecifier) defaultLineRange
           (func: SnapshotLineRange -> unit) =
    ...
    member x.GetStrictLineRangeSpecifier looseLineRangeSpecifier =
        ...
        | otherLineRangeSpecifier -> otherLineRangeSpecifier
    ...
    member x.GetStrictLineSpecifier looseLineSpecifier =
        ...
        | otherLineSpecifier -> otherLineSpecifier

    member x.RunWithPointAfterOrDefault (lineRangeSpecifier: LineRangeSpecifier) defaultLineRange
           (func: SnapshotPoint -> unit) =
    ...
    member x.RunScript lines =
        ...
        try
            while not parser.IsDone do
                ...
                _statusUtil.ContextLineNumber <- if lines.Length <> 1 then Some contextLineNumber else None
                x.RunLineCommand lineCommand |> ignore
        finally
        ...
        let rec inner (sb: System.Text.StringBuilder) sp =
            ...
            | SymbolicPathComponent.CurrentFileName modifiers :: tail ->
                let path =
                    ...
                    | FileNameModifier.PathFull :: tail ->
                        x.ApplyFileNameModifiers _vimBufferData.CurrentFilePath tail
            ...
            | SymbolicPathComponent.AlternateFileName(n, modifiers) :: tail ->
                ...
                let fileName =
                    if n >= fileHistory.Count then None else Some fileHistory.Items.[n]
                ...
                let path =
                    ...
                    | _ ->
                        let relativeFileName =
                            match fileName with
                            | None -> None
                            | Some filePath -> SystemUtil.StripPathPrefix x.CurrentDirectory filePath |> Some
    ...
    interface IVimInterpreter with
        ...
        member x.TryExpandCommandName(shortCommandName, commandName: outref<string>) =
            x.TryExpandCommandName(shortCommandName, &commandName)

[<Export(typeof<IVimInterpreterFactory>)>]
type VimInterpreterFactory [<ImportingConstructor>] (_commonOperationsFactory: ICommonOperationsFactory, _foldManagerFactory: IFoldManagerFactory, _bufferTrackingService: IBufferTrackingService) =
