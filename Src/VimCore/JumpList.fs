...
/// window
type internal JumpList(_textView: ITextView, _bufferTrackingService: IBufferTrackingService) =
    ...
    member x.LastJumpLocation =
        _lastJumpLocation |> OptionUtil.map2 (fun trackingLineColumn -> trackingLineColumn.VirtualPoint)
    ...
    member x.FindNodeTrackingLine lineNumber =
        ...
        if _list.Count = 0 then None else inner _list.First
        ...
        let trackingLineColumn =
            ...
            | None ->
                ...
                _bufferTrackingService.CreateLineOffset _textBuffer lineNumber offset
                    LineColumnTrackingMode.SurviveDeletes
            | Some node ->
    ...
    member x.Clear() =
        ...
        _list |> Seq.iter (fun trackingLineColumn -> trackingLineColumn.Close())
        _list.Clear()
    ...
    member x.MoveNewer count =
        ...
        | Some(current, index) ->
            ...
            if success then _current <- Some(current, index - count)
            success
    ...
    member x.SetLastJumpLocation lineNumber offset =
        ...
        _lastJumpLocation <-
            Some
                (_bufferTrackingService.CreateLineOffset _textView.TextBuffer lineNumber offset
