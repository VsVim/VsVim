...
type internal EditorSearchService(_textSearchService: ITextSearchService) =
    ...
    member private x.AddIntoCache (findData: FindData) (position: int) (foundSpan: SnapshotSpan) =
        lock (_cacheArray) (fun () ->
            ...
            _cacheArrayIndex <-
                ...
                if index >= _cacheArray.Length then 0 else index)
        ...
        while needsValidation do
            ...
            | Some span ->
                ...
                if span.Start = lastLine.Start && SnapshotLineUtil.IsPhantomLine lastLine then
                    ...
                    let matchesResult = x.GetMatchAtPosition findData matchPosition |> Option.isSome
                    ...
                    if not matchesResult then
                        ...
                        let newPosition =
                            if isForward then
                                if newPosition = endPoint.Position then 0 else newPosition + 1
                            else
                                let newPosition =
                                    if newPosition = 0 then endPoint.Position else newPosition - 1

                                let newPoint = SnapshotPoint(snapshot, newPosition)

                                let newPosition =
                                    if SnapshotPointUtil.IsInsideLineBreak newPoint
                                    then newPosition - 1
                                    else newPosition
                                newPosition
    ...
    member x.GetMatchAtPosition (findData: FindData) (position: int) =
        ...
        | Some matchSpan ->
            if matchSpan.Start.Position = position then Some matchSpan else None
        | None -> None
...
[<UsedInBackgroundThread>]
type internal SearchService(_textSearchService: ITextSearchService, _globalSettings: IVimGlobalSettings) =
    ...
    member x.GetServiceSearchData searchData navigator =
        { SearchData = searchData
          VimRegexOptions = _vimRegexOptions
          Navigator = navigator }
    ...
    member x.ApplySearchOffsetData (serviceSearchData: ServiceSearchData) (span: SnapshotSpan): SnapshotSpan option =
        ...
        | SearchOffsetData.End count ->
            x.ApplySearchOffsetDataStartEnd (SnapshotSpanUtil.GetLastIncludedPointOrStart span) count |> Some
    ...
    static member ConvertToFindDataCore (serviceSearchData: ServiceSearchData) snapshot =
        ...
        let textResult, textOptions, hadCaseSpecifier =
            let useRegex() =
                ...
                | VimResult.Error msg -> VimResult.Error msg, FindOptions.None, false
            ...
            | Some word ->
                ...
                let isBugPattern = searchData.Kind.IsAnyBackward && String.length word <= 2

                if isBugPattern || not isSimplePattern
                then useRegex()
                else VimResult.Result word, FindOptions.WholeWord, false
        ...
        let caseOptions =
            ...
            if hadCaseSpecifier then
                ...
                let hasUpper() =
                    pattern
                    |> Seq.filter CharUtil.IsLetter
                    |> Seq.filter CharUtil.IsUpper
                    |> SeqUtil.isNotEmpty
                if Util.IsFlagSet searchOptions SearchOptions.ConsiderSmartCase && smartCase && hasUpper()
                then FindOptions.MatchCase
                else FindOptions.None
            else
                FindOptions.MatchCase
        let revOptions =
            if searchData.Kind.IsAnyBackward then FindOptions.SearchReverse else FindOptions.None
        ...
        with :? System.ArgumentException as ex -> VimResult.Error ex.Message

    /// This is the core find function.  It will repeat the FindData search 'count' times.
    member x.FindCore (serviceSearchData: ServiceSearchData) (findData: FindData) (startPoint: SnapshotPoint) count
           includeStartPoint: SearchResult =
        ...
        let getNextSearchPosition (span: SnapshotSpan) =
            ...
            if isForward then
                ...
                if span.Length = 0
                then if span.End.Position = endPoint.Position then 0 else (span.Start.Add 1).Position
                else span.End.Position
            else if span.Start.Position = 0 then
                endPoint.Position
            else
    ...
    member x.FindNextCore (serviceSearchData: ServiceSearchData) (startPoint: SnapshotPoint) count =
        ...
        let includeStartPoint = Util.IsFlagSet serviceSearchData.SearchData.Options SearchOptions.IncludeStartPoint
    ...
    interface ISearchService with
        ...
        member x.FindNextPattern point searchData navigator count =
            x.FindNextPattern searchData point navigator count
