...
type Mapper(_keyInputSet: KeyInputSet, _keyMappings: KeyMapping seq, _globalSettings: IVimGlobalSettings, _isZeroMappingEnabled: bool, _overrideAllowRemap: bool option) =
    ...
    member x.GetKeyMapping() =
        ...
        let successfulMap (mappedKeyInputSet: KeyInputSet) (remainingKeyInputSet: KeyInputSet) =
            if remainingKeyInputSet.Length = 0
            then result := KeyMappingResult.Mapped mappedKeyInputSet
            else result := KeyMappingResult.PartiallyMapped(mappedKeyInputSet, remainingKeyInputSet)
        ...
        let processMapping lhs keyMapping =
            ...
            if depth.Value = _globalSettings.MaxMapDepth then
                ...
                if mappedSet.Value.Length > 0 then successfulMap mapped remaining
        ...
        while not isDone.Value do
            ...
            | [| keyMapping |] ->
                if keyMapping.Left = lhs then processMapping lhs keyMapping else needsMoreInput()
            | _ ->
        ...
        let notation =
            if notation.IndexOf("<leader>", StringComparison.OrdinalIgnoreCase) >= 0 then
                let replace =
                    ...
                    if found then value.StringValue else "\\"
...
type internal LocalKeyMap(_globalKeyMap: IVimGlobalKeyMap, _globalSettings: IVimGlobalSettings, _variableMap: VariableMap) =
    ...
    member x.GetKeyMappings(mode, includeGlobal) =
        ...
        if includeGlobal
        then Seq.append (_globalKeyMap.GetKeyMappings(mode)) local
        else local
    ...
    member x.Map (keyInputSet: KeyInputSet) allowRemap mode =
        ...
        let keyMappings =
            ...
            if Seq.isEmpty localKeyMappings then
                ...
                let newGlobalKeyMappings =
                    globalKeyMappings |> Seq.where (fun g -> Option.isNone (_map.Get(g.Left, mode)))
...
type internal GlobalAbbreviationMap() =
    ...
    interface IVimGlobalAbbreviationMap with
        ...
        member x.AddAbbreviation(lhs, rhs, allowRemap, mode) =
            x.Map.Add(lhs, mode, (Abbreviation(lhs, rhs, allowRemap, mode)))
...
type internal LocalAbbreviationMap(_keyMap: IVimLocalKeyMap, _globalAbbreviationMap: IVimGlobalAbbreviationMap, _wordUtil: WordUtil) =
    ...
    member x.GetAbbreviations(mode, includeGlobal) =
        let local = _map.Get(mode)
        if includeGlobal
        then Seq.append (_globalAbbreviationMap.GetAbbreviations(mode)) local
        else local
    ...
    member x.Parse text =
        if StringUtil.IsNullOrEmpty text || Seq.exists CharUtil.IsWhiteSpace text then
            ...
            if isLastKeyword && Seq.forall x.IsNonKeywordChar rest
            then Some AbbreviationKind.EndId
            elif not isLastKeyword
            then Some AbbreviationKind.NonId
            else None
    ...
    member x.Abbreviate (text: string) (triggerKeyInput: KeyInput) mode =
            ...
            if abbreviation.AllowRemap then
                ...
                let result =
                    ...
                    | KeyMappingResult.NeedsMoreInput _ ->
                        ...
                        let newResult =
                            _keyMap.Map
                                ((abbreviation.Replacement.Add triggerKeyInput), allowRemap = false, mode = remapMode)
        ...
        let getFullId() =
            ...
            | 1 ->
                if index = 0 then
                    Some(fullIdText, AbbreviationKind.FullId)
                else
                    let before = text.[index - 1]
                    let isValid = before = ' ' || before = '\t' || before = '\n' || before = '\r'
                    if isValid
                    then Some(fullIdText, AbbreviationKind.FullId)
                    else None
        ...
        if isAbbreviationTrigger && not (StringUtil.IsNullOrEmpty text) then
            match getFullId()
                  |> Option.orElseWith getEndId
                  |> Option.orElseWith getNonId with
            | None -> None
            | Some(keyText, kind) ->
