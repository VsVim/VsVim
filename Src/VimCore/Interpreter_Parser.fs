...
module internal ParseResultUtil =
    ...
    let Map (parseResult: ParseResult<'T>) mapFunc = parseResult.Map mapFunc
...
type internal ParseResultBuilder(_parser: Parser, _errorMessage: string) =
    ...
    member x.Return(value: 'T) = ParseResult.Succeeded value
    ...
    member x.Return(msg: string) = ParseResult.Failed msg

    member x.ReturnFrom value = value

    member x.Zero() = ParseResult.Failed _errorMessage

and LineCommandBuilder(_parser: Parser, _errorMessage: string) =
    ...
    member x.Return(value: LineCommand) = value
    ...
    member x.Return(msg: string) = _parser.ParseError msg

    member x.ReturnFrom value = value

    member x.Zero() = _parser.ParseError _errorMessage

and [<Sealed>] internal Parser(_globalSettings: IVimGlobalSettings, _vimData: IVimData) as this =
    ...
    member x.ParseTokenSequence texts =
        ...
        for text in texts do
            if _tokenizer.CurrentToken.TokenText = text then _tokenizer.MoveNextToken() else all <- false

        if not all then _tokenizer.MoveToMark mark

        all

    member x.ParseScriptLocalPrefix() =
        x.ParseTokenSequence [| "<"; "SID"; ">" |] || x.ParseTokenSequence [| "s"; ":" |]

    /// Reset the parser to the given set of input lines.
    member x.Reset(lines: string []) =
        _lines <- if lines.Length = 0 then [| "" |] else lines
        _lineIndex <- 0
        ...
        if x.IsCurrentLineBlank() then x.MoveToNextLine() |> ignore
    ...
    member x.IsCurrentLineBlank() =
        ...
        while not _tokenizer.IsAtEndOfLine && allBlank do
            if _tokenizer.CurrentTokenKind = TokenKind.Blank
            then _tokenizer.MoveNextToken()
            else allBlank <- false
    ...
    member x.ParseRestOfLineAsFilePath() =
        x.SkipBlanks()
        if _tokenizer.IsAtEndOfLine
        then []
        else x.ParseRestOfLine() |> x.ParseDirectoryPath
    ...
    member x.TryExpandCommandName name =
        ...
        let isAbbreviation (fullName: string) (abbreviation: string) =
            if name = fullName
            then true
            else name.StartsWith(abbreviation) && fullName.StartsWith(name)
    ...
    member x.ParseWhile predicate =
        ...
        if builder.Length = 0 then None else builder.ToString() |> Some
    ...
    member x.ParseMapClear allowBang keyRemapModes =
        ...
        if hasBang then
            if allowBang
            then LineCommand.ClearKeyMap([ KeyRemapMode.Insert; KeyRemapMode.Command ], mapArgumentList)
            else x.ParseError Resources.Parser_NoBangAllowed
        else
    ...
    member x.ParseMappingCore displayFunc mapFunc =
        ...
        | Some leftKeyNotation ->
            ...
            if StringUtil.IsBlanks rightKeyNotation
            then displayFunc (Some leftKeyNotation)
            else mapFunc leftKeyNotation rightKeyNotation
    ...
    member x.ParseAbbreviate(abbreviationModes, allowRemap) =
        ...
        x.ParseMappingCore (fun n -> LineCommand.DisplayAbbreviation(abbreviationModes, n))
            (fun l r -> LineCommand.Abbreviate(l, r, allowRemap, abbreviationModes, isLocal))
    ...
    member x.ParseMapKeysCore keyRemapModes allowRemap =
        x.SkipBlanks()
        let mapArgumentList = x.ParseMapArguments()
        x.ParseMappingCore (fun n -> LineCommand.DisplayKeyMap(keyRemapModes, n))
            (fun l r -> LineCommand.MapKeys(l, r, keyRemapModes, allowRemap, mapArgumentList))
    ...
    member x.ParseMapKeys allowBang keyRemapModes =
        ...
        if x.ParseBang() then
            if allowBang
            then x.ParseMapKeysCore [ KeyRemapMode.Insert; KeyRemapMode.Command ] true
            else x.ParseError Resources.Parser_NoBangAllowed
        else
    ...
    member x.ParseMapKeysNoRemap allowBang keyRemapModes =
        ...
        if x.ParseBang() then
            if allowBang
            then x.ParseMapKeysCore [ KeyRemapMode.Insert; KeyRemapMode.Command ] false
            else x.ParseError Resources.Parser_NoBangAllowed
        else
    ...
    member x.ParseMapUnmap allowBang keyRemapModes =
        ...
        if x.ParseBang() then
            if allowBang
            then inner [ KeyRemapMode.Insert; KeyRemapMode.Command ]
            else x.ParseError Resources.Parser_NoBangAllowed
        else
            inner keyRemapModes
        ...
        | TokenKind.Character '+' ->
            ...
            | TokenKind.Character c ->
                ...
                | LineCommand.ParseError _ ->
                    _tokenizer.MoveToMark mark
                    None
                | lineCommand -> CommandOption.ExecuteLineCommand lineCommand |> Some
            | _ ->
    ...
    member x.ParseRegisterName kind =
        ...
        if isGood then
            ...
            if Option.isSome name then _tokenizer.MoveNextChar()
            name
        else
            None
    ...
    member x.ParseSubstituteFlags() =
        ...
        let mutable flags =
            if _globalSettings.GlobalDefault then SubstituteFlags.ReplaceAll else SubstituteFlags.None
        ...
        while index < flagString.Length && Option.isNone parseResult do
            ...
            | 'g' ->
                ...
                if Util.IsFlagSet flags SubstituteFlags.ReplaceAll
                then flags <- flags &&& ~~~SubstituteFlags.ReplaceAll
                else flags <- flags ||| SubstituteFlags.ReplaceAll
            | '&' ->
                // The '&' flag is only legal in the first position.  After that
                // it terminates the flag notation
                if index = 0
                then flags <- flags ||| SubstituteFlags.UsePreviousFlags
                else parseResult <- ParseResult.Failed Resources.CommandMode_TrailingCharacters |> Some
            | _ ->
                // Illegal character in the flags string
                parseResult <- ParseResult.Failed Resources.CommandMode_TrailingCharacters |> Some
    ...
    member x.MergeLineRangeWithCommand lineRange lineCommand =
        ...
        | _ ->
            ...
            | LineCommand.Global(_, pattern, matchPattern, lineCommand) ->
            ...
            | LineCommand.HorizontalSplit(_, fileOptions, commandOptions) ->
            ...
            | LineCommand.QuitWithWrite(_, hasBang, fileOptions, filePath) ->
            ...
            | LineCommand.ReadFile(_, fileOptionList, filePath) ->
            ...
            | LineCommand.Substitute(_, pattern, replace, flags) ->
                LineCommand.Substitute(lineRange, pattern, replace, flags)
            | LineCommand.SubstituteRepeat(_, substituteFlags) ->
            ...
            | LineCommand.VerticalSplit(_, fileOptions, commandOptions) ->
            ...
            | LineCommand.Write(_, hasBang, fileOptionList, filePath) ->
    ...
    member x.ParseAutoCommand() =
        ...
        if isRemove then
            ...
            let onRemoveAll() = onRemoveEx List.empty List.empty ""
            ...
            | _ ->
                ...
                | ParseResult.Succeeded eventKindList ->
                    x.SkipBlanks()
                    let patternList =
                        if _tokenizer.IsAtEndOfLine then List.empty else parsePatternList()
        ...
        | TokenKind.Word name ->
            ...
            let callInfo =
                { LineRange = lineRange
                  ...
                  IsScriptLocal = isScriptLocal }
            if createEachTime
            then LineCommand.CSharpScriptCreateEachTime callInfo
            else LineCommand.CSharpScript callInfo
        | _ -> x.ParseError Resources.Parser_Error
        ...
        if x.ParseBang() then
            ...
            while not _tokenizer.IsAtEndOfLine && not hadError.Value do
                ...
                if _tokenizer.CurrentChar = '-' then
                    ...
                    if CharUtil.IsLetter c && CharUtil.IsLetter o && c < o
                    then parseRange c o
                    elif CharUtil.IsDigit c && CharUtil.IsDigit o && c < o
                    then parseRange c o
                    else hadError := true
                else
                    match Mark.OfChar c with
    ...
    member x.ParseFunctionStart() =
        ...
        let parseModifiers() =
            ...
            let rec inner isAbort isDict isRange =
                ...
                | TokenKind.EndOfLine -> (isAbort, isDict, isRange, false)
                | _ ->
                    _tokenizer.MoveToEndOfLine()
    ...
    member x.ParseFunctionEnd() = LineCommand.FunctionEnd
    ...
    member x.ParseLineSpecifier() =
        ...
        let lineSpecifier =
            if _tokenizer.CurrentChar = '.' then
                ...
                | TokenKind.Number _ ->
                    ...
                    |> Some
                | _ -> LineSpecifier.CurrentLine |> Some
        ...
        | Some lineSpecifier ->
            ...
            if _tokenizer.CurrentChar = '+' then parseAdjustment false
            elif _tokenizer.CurrentChar = '-' then parseAdjustment true
            else Some lineSpecifier
    ...
    member x.ParseLineRange(): LineRangeSpecifier =
        if _tokenizer.CurrentChar = '%' then
            ...
            let startLine =
                ...
                | None ->
                    if _tokenizer.CurrentChar = ',' || _tokenizer.CurrentChar = ';'
                    then Some LineSpecifier.CurrentLine
                    else None
                | Some left -> startLine
            match startLine with
    ...
    member x.ParseSubstitute lineRange processFlags =
        ...
        let isValidDelimiter c =
            let isBad = CharUtil.IsLetter c || CharUtil.IsDigit c || c = '\\' || c = '"' || c = '|'
            not isBad
    ...
    member x.ParseOptionBackslash isFileName =
        ...
        if isFileName then SystemUtil.ResolvePath value else value
    ...
    member x.ParseQuitAndWrite lineRange =
        ...
        x.SkipBlanks()
        let fileName =
            if _tokenizer.IsAtEndOfLine then None else x.ParseRestOfLine() |> Some
    ...
    member x.ParseEnvironmentVariableName() =
        ...
        | TokenKind.Word word -> Expression.EnvironmentVariableName word |> ParseResult.Succeeded
    ...
    member x.ParseFiles() = LineCommand.Files

    /// Parse out the fold command
    member x.ParseFold lineRange = LineCommand.Fold lineRange
    ...
    member x.ParseVimGrep count =
        ...
        if _tokenizer.IsAtEndOfLine then
            ...
            | Some pattern -> parseFilePattern pattern VimGrepFlags.None
            | None -> x.ParseError Resources.Parser_Error
        else
            ...
            if foundDelimiter then
                ...
                | ParseResult.Failed message -> x.ParseError message
                | ParseResult.Succeeded flags -> parseFilePattern pattern flags
            else
    ...
    member x.ParseHistory() = LineCommand.History
    ...
    member x.ParseIfStart() = x.ParseIfOrElseIf LineCommand.IfStart
    ...
    member x.ParseElseIf() = x.ParseIfOrElseIf LineCommand.ElseIf
    ...
    member x.ParseJoin lineRange =
        ...
        let joinKind =
            if hasBang then JoinKind.KeepEmptySpaces else JoinKind.RemoveEmptySpaces
    ...
    member x.ParseLet() =
        ...
        if _tokenizer.CurrentTokenKind = TokenKind.EndOfLine then
            ...
            | ParseResult.Succeeded(Expression.VariableName variableName) ->
                ...
                then parseAssignment variableName (fun lhs rhs -> LineCommand.Let(lhs, rhs))
                else parseDisplayLet variableName
    ...
    member x.ParsePut lineRange =
        ...
        if hasBang
        then LineCommand.PutBefore(lineRange, registerName)
        else LineCommand.PutAfter(lineRange, registerName)
    ...
    member x.ParseSort lineRange =
        ...
        | ParseResult.Succeeded flags ->
            ...
            | _ -> LineCommand.Sort(lineRange, hasBang, flags, None)
    ...
    member x.ParseStopInsert() = LineCommand.StopInsert
        ...
        while more do
            ...
            if _tokenizer.IsAtEndOfLine then
                ...
                if _tokenizer.IsAtEndOfLine then
                    ...
                    | None -> result <- x.ParseError Resources.CommandMode_InvalidCommand |> Some

        match result with
        | Some lineCommand -> lineCommand
        | None ->
    ...
    member x.ParseSingleLine() =
            ...
            let lineCommand =
                if lineCommand.Failed then
                    ...
                    if _tokenizer.IsAtEndOfLine then
                        ...
                        if nextCommand.Failed
                        then nextCommand
                        else LineCommand.Compose(lineCommand, nextCommand)
                    else
        ...
        let doParse name =
            let parseResult =
                ...
                | "cnoreabbrev" ->
                    noRange (fun () -> x.ParseAbbreviate([ AbbreviationMode.Command ], allowRemap = false))
                ...
                | "inoreabbrev" ->
                    noRange (fun () -> x.ParseAbbreviate([ AbbreviationMode.Insert ], allowRemap = false))
                ...
                | "map" ->
                    noRange
                        (fun () ->
                            x.ParseMapKeys true
                                [ KeyRemapMode.Normal; KeyRemapMode.Visual; KeyRemapMode.Select; KeyRemapMode.OperatorPending ])
                | "mapclear" ->
                    noRange
                        (fun () ->
                            x.ParseMapClear true
                                [ KeyRemapMode.Normal; KeyRemapMode.Visual; KeyRemapMode.Command; KeyRemapMode.OperatorPending ])
                ...
                | "vmapclear" ->
                    noRange (fun () -> x.ParseMapClear false [ KeyRemapMode.Visual; KeyRemapMode.Select ])
                | "vscmd" -> x.ParseHostCommand()
                | "vsplit" -> x.ParseSplit LineCommand.VerticalSplit lineRange
                | "vnoremap" ->
                    noRange (fun () -> x.ParseMapKeysNoRemap false [ KeyRemapMode.Visual; KeyRemapMode.Select ])
    ...
    member x.ParseOptionName() =
        ...
        _tokenizer.MoveNextToken()
        match tokenKind with
        | TokenKind.Word word -> Expression.OptionName word |> ParseResult.Succeeded
    ...
    member x.ParseDictionary() =
        ...
        | TokenKind.Character '}' ->
            _tokenizer.MoveNextToken()
            VariableValue.Dictionary Map.empty
            |> Expression.ConstantValue
            |> ParseResult.Succeeded
        | _ -> ParseResult.Failed Resources.Parser_Error
    ...
    member x.ParseSingleExpression() =
        ...
        use reset = _tokenizer.SetTokenizerFlagsScoped TokenizerFlags.AllowDoubleQuote
        match _tokenizer.CurrentTokenKind with
        | TokenKind.Character '\"' -> x.ParseStringConstant()
        | TokenKind.Character '\'' -> x.ParseStringLiteral()
        | TokenKind.Character '&' -> x.ParseOptionName()
        | TokenKind.Character '[' -> x.ParseList()
        | TokenKind.Character '{' -> x.ParseDictionary()
        | TokenKind.Character '$' -> x.ParseEnvironmentVariableName()
        | TokenKind.Character '@' ->
            _tokenizer.MoveNextToken()
            match x.ParseRegisterName ParseRegisterName.All with
        ...
        | _ ->
            match x.ParseVariableName() with
            | ParseResult.Failed msg -> ParseResult.Failed msg
            | ParseResult.Succeeded variable ->
                x.SkipBlanks()
                match _tokenizer.CurrentTokenKind with
                | TokenKind.Character '(' ->
                    match x.ParseFunctionArguments true with
                    | ParseResult.Succeeded args -> Expression.FunctionCall(variable, args) |> ParseResult.Succeeded
                    | ParseResult.Failed msg -> ParseResult.Failed msg
    ...
    member x.ParseFunctionArguments atBeginning =
        ...
        | _ ->
            if atBeginning
            then recursivelyParseArguments()
    ...
    member x.ParseNextCommand() = x.ParseSingleCommand()
    ...
    member x.ParseLineCommands lines =
        ...
        let rec inner rest =
            ...
            if not x.IsDone
            then inner (fun item -> rest (lineCommand :: item))
            else rest [ lineCommand ]
        inner (fun all -> all)
            ...
            | TokenKind.Character ':' ->
                ...
                | Some m ->
                    ...
                    | FileNameModifier.PathFull when modifiers.IsEmpty ->
                        ...
                        inner (m :: modifiers)
                    | FileNameModifier.Tail when not
                                                     (List.exists (fun m ->
                                                         m = FileNameModifier.Root || m = FileNameModifier.Extension
                                                         || m = FileNameModifier.Tail) modifiers) ->
                        // 't' must precede 'r' and 'e' and cannot be repeated -- violations end the modifier sequence
                        inner (m :: modifiers)
                    | FileNameModifier.Head when not
                                                     (List.exists (fun m ->
                                                         m = FileNameModifier.Root || m = FileNameModifier.Extension
                                                         || m = FileNameModifier.Tail) modifiers) ->
        ...
        let rec inner components =
            if _tokenizer.IsAtEndOfLine then
                ...
                | TokenKind.Character '\\' ->
                    ...
                    | _ -> inner (SymbolicPathComponent.Literal "\\" :: components)
                | TokenKind.Character '%' ->
                    _tokenizer.MoveNextChar()
                ...
                | _ ->
                    ...
                    let nextComponents =
                        ...
                        | SymbolicPathComponent.Literal lhead :: tail ->
                            (SymbolicPathComponent.Literal(lhead + literal)) :: tail
                        | _ -> (SymbolicPathComponent.Literal literal :: components)
                    inner nextComponents

        List.rev (inner List.Empty)

and ConditionalParser(_parser: Parser, _initialExpr: Expression) =
    ...
    member x.IsDone = _parser.IsDone
    ...
    member x.Parse() =
        ...
        while not _parser.IsDone && not isDone && Option.isNone error do
            ...
            | LineCommand.Else ->
                if _state = StateAfterElse then error <- Some Resources.Parser_MultipleElse
                x.CreateConditionalBlock()
                _state <- StateAfterElse
            | LineCommand.ElseIf expr ->
                if _state = StateAfterElse then error <- Some Resources.Parser_ElseIfAfterElse
                x.CreateConditionalBlock()
