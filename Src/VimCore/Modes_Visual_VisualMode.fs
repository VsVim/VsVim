...
type internal VisualMode(_vimBufferData: IVimBufferData, _operations: ICommonOperations, _motionUtil: IMotionUtil, _visualKind: VisualKind, _runner: ICommandRunner, _capture: IMotionCapture, _selectionTracker: ISelectionTracker) =
    ...
    static let SharedCommands =
        let visualSeq =
            seq {
                yield ("c", CommandFlags.Repeatable ||| CommandFlags.LinkedWithNextCommand,
                       VisualCommand.ChangeSelection)
                yield ("C", CommandFlags.Repeatable ||| CommandFlags.LinkedWithNextCommand,
                       VisualCommand.ChangeLineSelection true)
                ...
                yield ("P", CommandFlags.Repeatable, VisualCommand.PutOverSelection false)
                yield ("R", CommandFlags.Repeatable ||| CommandFlags.LinkedWithNextCommand,
                       VisualCommand.ChangeLineSelection false)
                yield ("s", CommandFlags.Repeatable ||| CommandFlags.LinkedWithNextCommand,
                       VisualCommand.ChangeSelection)
                yield ("S", CommandFlags.Repeatable ||| CommandFlags.LinkedWithNextCommand,
                       VisualCommand.ChangeLineSelection false)
        ...
        let complexSeq =
            seq
                {
                yield ("r", CommandFlags.Repeatable,
                       BindData<_>.CreateForKeyInput KeyRemapMode.Language VisualCommand.ReplaceSelection) }
        ...
        let normalSeq =
            seq {
                ...
                yield (":", CommandFlags.Special,
                       NormalCommand.SwitchMode
                           (ModeKind.Command, (ModeArgument.PartialCommand CommandFromVisualModeString)))
            }
            |> Seq.map (fun (str, flags, command) ->
                let keyInputSet = KeyNotationUtil.StringToKeyInputSet str
            ...
            if keyInput = KeyInputUtil.EscapeKey && x.ShouldHandleEscape then
                ...
                | BindResult.Complete commandRanData ->
                    ...
                    if Util.IsFlagSet commandRanData.CommandBinding.CommandFlags CommandFlags.ResetAnchorPoint then
                        match _vimBufferData.VisualAnchorPoint
                              |> OptionUtil.map2 (TrackingPointUtil.GetPoint x.CurrentSnapshot) with
                        | None -> ()
                        | Some anchorPoint ->
                ...
                | BindResult.Error ->
                    _selectionTracker.UpdateSelection()
                    _operations.Beep()
                    if keyInput.IsMouseKey
                    then ProcessResult.NotHandled
                    else ProcessResult.Handled ModeSwitch.NoSwitch
    ...
    member x.ResetCaret() =
        ...
        | Some startPoint ->
            ...
            let point =
                if startPoint.Position < x.CaretPoint.Position
                then startPoint
                else x.CaretPoint
