...
type internal CommandRunner(_vimBufferData: IVimBufferData, _motionCapture: IMotionCapture, _commandUtil: ICommandUtil, _visualKind: VisualKind, _defaultKeyRemapMode: KeyRemapMode) =
    ...
    member x.BindCount (initialDigit: KeyInput) completeFunc =
        ...
        let rec inner (num: string) (ki: KeyInput) =
            if ki.IsDigit then
                ...
                BindResult.NeedMoreInput
                    { KeyRemapMode = _defaultKeyRemapMode
                      BindFunction = inner num }
            else
                _inCount <- false
    ...
    member x.BindCountAndRegister(keyInput: KeyInput) =
        ...
        let tryRegister (keyInput: KeyInput) foundFunc missingFunc =
            if keyInput.Char = '"' then x.BindRegister foundFunc else missingFunc keyInput

        let tryCount (keyInput: KeyInput) foundFunc missingFunc =
            if keyInput.IsDigit && keyInput.Char <> '0' then x.BindCount keyInput foundFunc else missingFunc keyInput
        ...
        let tryCountThenRegister keyInput missingCount =
            let foundCount count keyInput =
                ...
                tryRegister keyInput (fun register -> bindCommandResult (Some register) count)
                    (fun keyInput -> x.BindCommand None count keyInput)
        ...
        let tryRegisterThenCount keyInput missingRegister =
            let foundRegister register =
                ...
                let next keyInput =
                    tryCount keyInput (fun count keyInput -> x.BindCommand register (Some count) keyInput)
                        (fun keyInput -> x.BindCommand register None keyInput)
                BindResult.NeedMoreInput
                    { KeyRemapMode = _defaultKeyRemapMode
                      BindFunction = next }
        ...
        tryRegisterThenCount keyInput
            (fun keyInput -> tryCountThenRegister keyInput (fun keyInput -> x.BindCommand None None keyInput))
    ...
    member x.BindMotion (commandBinding: CommandBinding) (commandData: CommandData) motionFunc keyInput =
        ...
        let convertMotion motion motionCount =
            let argument =
                MotionArgument
                    (MotionContext.AfterOperator, operatorCount = commandData.Count, motionCount = motionCount)
            ...
            | Some commandBinding ->
                ...
                | CommandBinding.MotionBinding(_, _, func) ->
                    ...
                    if Seq.isEmpty withPrefix then
                        ...
                        let bindData =
                            { KeyRemapMode = KeyRemapMode.OperatorPending
                              BindFunction = (fun keyInput -> x.BindMotion commandBinding commandData func keyInput) }
                ...
                | CommandBinding.ComplexNormalBinding(_, _, bindDataStorage) ->
                    ...
                    let bindData =
                        bindData.Convert
                            (fun normalCommand -> (Command.NormalCommand(normalCommand, commandData), commandBinding))
                ...
                | CommandBinding.ComplexVisualBinding(_, _, bindDataStorage) ->
                    ...
                    let bindData =
                        bindData.Map(fun visualCommand ->
                            let visualSpan = x.VisualSpan
                            let visualCommand = Command.VisualCommand(visualCommand, commandData, visualSpan)
                            BindResult.Complete(visualCommand, commandBinding))
                    BindResult.NeedMoreInput bindData
            | None ->
                ...
                if commandName.KeyInputs.Length > 1 && not hasPrefixMatch then
                    ...
                    | Some command ->
                        ...
                        | CommandBinding.NormalBinding _ -> bindNext KeyRemapMode.None
                        | CommandBinding.VisualBinding _ -> bindNext KeyRemapMode.None
                        | CommandBinding.InsertBinding _ -> bindNext KeyRemapMode.None
                        | CommandBinding.ComplexNormalBinding _ -> bindNext KeyRemapMode.None
                        | CommandBinding.ComplexVisualBinding _ -> bindNext KeyRemapMode.None
                    | None ->
                ...
                let bindData =
                    ...
                    | Some oldBindData ->
                        ...
                        if isNone && wasPending then
                            { KeyRemapMode = oldBindData.KeyRemapMode
                              BindFunction = bindData.BindFunction }
                        else
                            bindData

                _runBindData <- Some bindData
                BindResult.NeedMoreInput
                    { KeyRemapMode = bindData.KeyRemapMode
                      BindFunction = x.Run }
