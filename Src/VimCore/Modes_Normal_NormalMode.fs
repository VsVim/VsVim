...
type internal NormalMode(_vimBufferData: IVimBufferData, _operations: ICommonOperations, _motionUtil: IMotionUtil, _runner: ICommandRunner, _capture: IMotionCapture, _incrementalSearch: IIncrementalSearch) as this =
    ...
    static let SharedStandardCommands =
        let normalSeq =
            seq {
                yield ("a", CommandFlags.LinkedWithNextCommand ||| CommandFlags.Repeatable,
                       NormalCommand.InsertAfterCaret)
                yield ("A", CommandFlags.LinkedWithNextCommand ||| CommandFlags.Repeatable,
                       NormalCommand.InsertAtEndOfLine)
                yield ("C", CommandFlags.LinkedWithNextCommand ||| CommandFlags.Repeatable,
                       NormalCommand.ChangeTillEndOfLine)
                ...
                yield ("gugu", CommandFlags.Repeatable,
                       NormalCommand.ChangeCaseCaretLine ChangeCharacterKind.ToLowerCase)
                yield ("guu", CommandFlags.Repeatable, NormalCommand.ChangeCaseCaretLine ChangeCharacterKind.ToLowerCase)
                yield ("gUgU", CommandFlags.Repeatable,
                       NormalCommand.ChangeCaseCaretLine ChangeCharacterKind.ToUpperCase)
                ...
                yield ("I", CommandFlags.LinkedWithNextCommand ||| CommandFlags.Repeatable,
                       NormalCommand.InsertAtFirstNonBlank)
                yield ("J", CommandFlags.Repeatable, NormalCommand.JoinLines JoinKind.RemoveEmptySpaces)
                yield ("o", CommandFlags.LinkedWithNextCommand ||| CommandFlags.Repeatable,
                       NormalCommand.InsertLineBelow)
                yield ("O", CommandFlags.LinkedWithNextCommand ||| CommandFlags.Repeatable,
                       NormalCommand.InsertLineAbove)
                ...
                yield ("s", CommandFlags.LinkedWithNextCommand ||| CommandFlags.Repeatable,
                       NormalCommand.SubstituteCharacterAtCaret)
        ...
        let motionSeq =
            seq {
                yield ("c", CommandFlags.Change ||| CommandFlags.LinkedWithNextCommand ||| CommandFlags.Repeatable,
                       NormalCommand.ChangeMotion)
                yield ("d", CommandFlags.Repeatable ||| CommandFlags.Delete, NormalCommand.DeleteMotion)
                yield ("gU", CommandFlags.Repeatable,
                       (fun motion -> NormalCommand.ChangeCaseMotion(ChangeCharacterKind.ToUpperCase, motion)))
                yield ("gu", CommandFlags.Repeatable,
                       (fun motion -> NormalCommand.ChangeCaseMotion(ChangeCharacterKind.ToLowerCase, motion)))
                yield ("g?", CommandFlags.Repeatable,
                       (fun motion -> NormalCommand.ChangeCaseMotion(ChangeCharacterKind.Rot13, motion)))
                yield ("g~", CommandFlags.Repeatable,
                       (fun motion -> NormalCommand.ChangeCaseMotion(ChangeCharacterKind.ToggleCase, motion)))
        ...
        Seq.append normalSeq motionSeq |> List.ofSeq
    ...
    member x.KeyRemapMode =
        if _runner.IsWaitingForMoreInput then _runner.KeyRemapMode else KeyRemapMode.Normal
        ...
        if not x.IsCommandRunnerPopulated then
            ...
            let complexSeq =
                seq {
                    ...
                    yield ("m", CommandFlags.Special,
                           bindDataToStorage
                               (BindData<_>.CreateForChar KeyRemapMode.None NormalCommand.SetMarkToCaret))
                    yield ("@", CommandFlags.Special,
                           bindDataToStorage (BindData<_>.CreateForChar KeyRemapMode.None NormalCommand.RunAtCommand))
                }
            ...
            _selectionAlternateCommands <-
                _runner.Commands
                |> Seq.filter
                    (fun commandBinding -> Set.contains commandBinding.KeyInputSet _sharedSelectionCommandNameSet)
                |> List.ofSeq
    ...
    member x.OnGlobalSettingsChanged(args: SettingEventArgs) =
        if x.IsCommandRunnerPopulated then
            ...
            if StringUtil.IsEqual setting.Name GlobalSettingNames.TildeOpName
            then x.UpdateTildeCommand()
            elif StringUtil.IsEqual setting.Name GlobalSettingNames.KeyModelName
            then x.UpdateSelectionCommands()
    ...
    member x.GetTildeCommand() =
        ...
        let command =
            if _globalSettings.TildeOp then
                CommandBinding.MotionBinding
                    (name, flags, (fun motion -> NormalCommand.ChangeCaseMotion(ChangeCharacterKind.ToggleCase, motion)))
            else
                CommandBinding.NormalBinding
                    (name, flags, NormalCommand.ChangeCaseCaretPoint ChangeCharacterKind.ToggleCase)
        name, command
    ...
    member x.UpdateSelectionCommands() =
        ...
        let commandList =
            ...
            then _sharedSelectionCommands
            else _selectionAlternateCommands
