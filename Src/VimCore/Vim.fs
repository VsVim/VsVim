...
type internal BulkOperations [<ImportingConstructor>] (_vimHost: IVimHost) =
    ...
    member x.BeginBulkOperation() =
        ...
        if _bulkOperationCount = 0 then _vimHost.BeginBulkOperation()
        ...
        { new System.IDisposable with
            member x.Dispose() =
                _bulkOperationCount <- _bulkOperationCount - 1
                if _bulkOperationCount = 0 then _vimHost.EndBulkOperation() }
...
type internal VimBufferFactory [<ImportingConstructor>] (_host: IVimHost, _editorOperationsFactoryService: IEditorOperationsFactoryService, _editorOptionsFactoryService: IEditorOptionsFactoryService, _outliningManagerService: IOutliningManagerService, _completionWindowBrokerFactoryService: IDisplayWindowBrokerFactoryService, _commonOperationsFactory: ICommonOperationsFactory, _lineChangeTrackerFactory: ILineChangeTrackerFactory, _textSearchService: ITextSearchService, _bufferTrackingService: IBufferTrackingService, _undoManagerProvider: ITextBufferUndoManagerProvider, _statusUtilFactory: IStatusUtilFactory, _foldManagerFactory: IFoldManagerFactory, _selectionUtilService: ISelectionUtilFactoryService, _keyboardDevice: IKeyboardDevice, _mouseDevice: IMouseDevice, _wordCompletionSessionFactoryService: IWordCompletionSessionFactoryService, _bulkOperations: IBulkOperations) =
    ...
    member x.CreateVimTextBuffer (textBuffer: ITextBuffer) (vim: IVim) =
        ...
        let localAbbreviationMap =
            LocalAbbreviationMap(localKeyMap, vim.GlobalAbbreviationMap, wordUtil) :> IVimLocalAbbreviationMap
        ...
        let undoRedoOperations =
            let history =
                ...
                if manager = null then None else manager.TextBufferUndoHistory |> Some
            UndoRedoOperations(_host, statusUtil, history, _editorOperationsFactoryService) :> IUndoRedoOperations

        VimTextBuffer
            (textBuffer, localAbbreviationMap, localKeyMap, localSettings, _bufferTrackingService, undoRedoOperations,
             wordUtil, vim)
    ...
    member x.CreateVimBuffer(vimBufferData: IVimBufferData) =
        ...
        let textChangeTracker =
            Modes.Insert.TextChangeTracker.GetTextChangeTracker vimBufferData _commonOperationsFactory
        ...
        let commandUtil =
            CommandUtil
                (vimBufferData, motionUtil, commonOperations, foldManager, insertUtil, _bulkOperations,
                 lineChangeTracker) :> ICommandUtil

        let bufferRaw =
            VimBuffer
                (vimBufferData, incrementalSearch, motionUtil, vimBufferData.VimTextBuffer.WordNavigator,
                 vimBufferData.WindowSettings, commandUtil)
        let buffer = bufferRaw :> IVimBuffer

        let vim = vimBufferData.Vim
        let createCommandRunner kind countKeyRemapMode =
            CommandRunner(vimBufferData, capture, commandUtil, kind, countKeyRemapMode) :> ICommandRunner
        ...
        let visualOptsFactory visualKind =
            Modes.Visual.SelectionTracker(vimBufferData, commonOperations, incrementalSearch, visualKind) :> Modes.Visual.ISelectionTracker
        ...
        let visualModeSeq =
            ...
            |> Seq.map (fun visualKind ->
                ...
                ((Modes.Visual.VisualMode
                    (vimBufferData, commonOperations, motionUtil, visualKind,
                     createCommandRunner visualKind KeyRemapMode.Visual, capture, tracker)) :> IMode))
        ...
        let selectModeSeq =
            ...
            |> Seq.map (fun visualKind ->
                ...
                Modes.Visual.SelectMode
                    (vimBufferData, commonOperations, motionUtil, visualKind, runner, capture, undoRedoOperations,
                     tracker) :> IMode)
            |> List.ofSeq
        ...
        let modeList =
            [ ((Modes.Normal.NormalMode
                    (vimBufferData, commonOperations, motionUtil,
                     createCommandRunner VisualKind.Character KeyRemapMode.Normal, capture, incrementalSearch)) :> IMode)
              ((Modes.Command.CommandMode(buffer, commonOperations)) :> IMode)
              ((Modes.Insert.InsertMode
                  (buffer, commonOperations, broker, editOptions, undoRedoOperations,
                   textChangeTracker :> Modes.Insert.ITextChangeTracker, insertUtil, motionUtil, commandUtil, capture,
                   false, _keyboardDevice, _mouseDevice, _wordCompletionSessionFactoryService)) :> IMode)
              ((Modes.Insert.InsertMode
                  (buffer, commonOperations, broker, editOptions, undoRedoOperations, textChangeTracker, insertUtil,
                   motionUtil, commandUtil, capture, true, _keyboardDevice, _mouseDevice,
                   _wordCompletionSessionFactoryService)) :> IMode)
...
[<Export(typeof<IVim>)>]
type internal Vim(_vimHost: IVimHost, _bufferFactoryService: IVimBufferFactory, _interpreterFactory: IVimInterpreterFactory, _bufferCreationListeners: Lazy<IVimBufferCreationListener> list, _globalSettings: IVimGlobalSettings, _markMap: IMarkMap, _clipboardDevice: IClipboardDevice, _search: ISearchService, _fileSystem: IFileSystem, _vimData: IVimData, _bulkOperations: IBulkOperations, _variableMap: VariableMap, _editorToSettingSynchronizer: IEditorToSettingsSynchronizer, _statusUtilFactory: IStatusUtilFactory, _commonOperationsFactory: ICommonOperationsFactory, _mouseDevice: IMouseDevice) as this =
    ...
    new(host: IVimHost, bufferFactoryService: IVimBufferFactory, interpreterFactory: IVimInterpreterFactory,
        bufferTrackingService: IBufferTrackingService,
        [<ImportMany>] bufferCreationListeners: Lazy<IVimBufferCreationListener> seq, search: ITextSearchService,
        fileSystem: IFileSystem, clipboard: IClipboardDevice, bulkOperations: IBulkOperations,
        editorToSettingSynchronizer: IEditorToSettingsSynchronizer, statusUtilFactory: IStatusUtilFactory,
        commonOperationsFactory: ICommonOperationsFactory, mouseDevice: IMouseDevice) =
        ...
        Vim
            (host, bufferFactoryService, interpreterFactory, listeners, globalSettings, markMap :> IMarkMap, clipboard,
             SearchService(search, globalSettings) :> ISearchService, fileSystem, vimData, bulkOperations, variableMap,
             editorToSettingSynchronizer, statusUtilFactory, commonOperationsFactory, mouseDevice)
    ...
    member x.IsDisabled
        ...
        and set value =
            ...
            _isDisabled <- value
            if changed then x.UpdatedDisabledMode()

    member x.VariableMap = _variableMap

    member x.VimBuffers =
        _vimBufferMap.Values
        |> Seq.map (fun (vimBuffer, _, _) -> vimBuffer)
        |> List.ofSeq
    ...
    member x.CloseAllVimBuffers() =
        x.VimBuffers
        |> List.iter (fun vimBuffer ->
            if not vimBuffer.IsClosed then vimBuffer.Close())
    ...
    member x.CreateVimTextBuffer (textBuffer: ITextBuffer) (localSettings: IVimLocalSettings option) =
        ...
        // as well
        if _isDisabled then vimTextBuffer.SwitchMode ModeKind.Disabled ModeArgument.None
    ...
    member x.CreateVimBufferCore textView (windowSettings: IVimWindowSettings option)
           (vimBufferData: IVimBufferData option) =
        if _vimBufferMap.ContainsKey(textView) then invalidArg "textView" Resources.Vim_TextViewAlreadyHasVimBuffer
    ...
    member x.LoadVimRcFileContents() =
        ...
        |> Seq.tryPick (fun vimRcPath ->
            if not (_vimHost.ShouldIncludeRcFile vimRcPath) then
                ...
                _fileSystem.ReadAllLines vimRcPath.FilePath |> Option.map (fun lines -> (vimRcPath, lines)))
    ...
    member x.LoadDefaultSettings(): unit =
        ...
        | _ -> _globalSettings.Backspace <- "indent,eol,start"
    ...
    member x.GetSessionDataFilePath() = Path.Combine(x.GetSessionDataDirectory(), "vimdata.json")
    ...
    member x.WriteSessionData (sessionData: SessionData) filePath =
        ...
        with ex -> VimTrace.TraceError ex

    member x.LoadSessionDataCore filePath =
        let sessionData = x.ReadSessionData filePath

        let registers =
            if sessionData.Registers = null then [||] else sessionData.Registers
        for sessionReg in registers do
            match sessionReg.Name |> RegisterName.OfChar with
            | Some name ->
                let kind =
                    if sessionReg.IsCharacterWise then OperationKind.CharacterWise else OperationKind.LineWise
    ...
    member x.SaveSessionDataCore filePath =
        let sessionRegisterArray =
            ...
            |> Seq.map (fun n ->
                ...
                { Name = n.Char
                  IsCharacterWise = isCharacterWise
                  Value = value
                  IsMacro = isMacro })
            |> Seq.toArray
    ...
    interface IVim with
        ...
        member x.CreateVimTextBuffer textBuffer =
            x.CreateVimTextBuffer textBuffer (Some(x.GetLocalSettingsForNewTextBuffer()))
        ...
        member x.TryGetOrCreateVimBufferForHost(textView, vimBuffer) =
            x.TryGetOrCreateVimBufferForHost(textView, &vimBuffer)
