...
type SnapshotWordUtil(_keywordCharSet: VimCharSet) =
    ...
    member x.GetFullWordSpanInText wordKind (text: string) index =
        ...
        | Some span ->
            if span.Start > 0 && Option.isSome (EditUtil.GetFullLineBreakSpanAtIndex text (span.Start - 1))
            then Some span
            else None
        | None ->
            let c = text.[index]
            if CharUtil.IsWhiteSpace c then
                ...
                let predicate =
                    match wordKind with
                    | WordKind.NormalWord ->
                        if x.IsNormalWordChar c then x.IsNormalWordChar else x.IsBigWordOnlyChar
    ...
    member x.GetFullWordSpan wordKind point =
        ...
        | Some span ->
            ...
            // for instance
            if span.Contains(point) then Some span else None
...
[<Class>]
[<Sealed>]
type SnapshotWordNavigator(_wordUtil: SnapshotWordUtil, _contentType: IContentType) =
    ...
    interface ITextStructureNavigator with
        ...
        member __.GetSpanOfFirstChild span = SnapshotSpan(span.End, 0)
        member __.GetSpanOfNextSibling span = SnapshotSpan(span.End, 0)
...
type WordUtil(_textBuffer: ITextBuffer, _localSettings: IVimLocalSettings) as this =
    ...
    do
        ...
        _textBuffer.ContentTypeChanged |> Observable.add (fun _ -> createNavigators())
