...
module KeyNotationUtil =
    ...
    let TryCharNotationToKeyInput(dataCharSpan: CharSpan) =
        ...
        let numberBase =
            if numberString.StartsWith("0x") then 16
            elif numberString.StartsWith("0") then 8
            else 10
    ...
    let TryStringToKeyInput(data: string) =
        ...
        | Some(keyInput, length) ->
            if length = data.Length then Some keyInput else None
        | None -> None
    ...
    let TryGetSpecialKeyName(keyInput: KeyInput) =
        let found =
            ...
            |> Seq.tryFind (fun pair ->
                ...
                specialInput.Key = keyInput.Key && specialInput.Char = keyInput.Char
    ...
    let GetDisplayName(keyInput: KeyInput) =
        ...
        let inner name keyModifiers forceBookend =
            ...
            then if forceBookend then sprintf "<%s>" name else name
            else sprintf "<%s%s>" prefix name
        ...
        let checkForSpecialControl() =
            ...
            | VimKey.RawCharacter, Some c ->
                ...
                | c -> inner (string (c)) keyInput.KeyModifiers false
