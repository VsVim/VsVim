...
// Mark glyph tag
type MarkGlyphTag(_char: char) =

    member x.Char = _char

    interface ITag

/// Tagger to display marks in the indicator margin
type MarkGlyphTaggerSource
    ( 
        _vimBufferData: IVimBufferData
    ) as this =

    let _markMap = _vimBufferData.Vim.MarkMap
    let _changed = StandardEvent()
    let _eventHandlers = DisposableBag()
    let mutable _currentMark: Mark option = Some (Mark.LocalMark (LocalMark.Letter Letter.A))

    static let EmptyTagList = ReadOnlyCollection<ITagSpan<MarkGlyphTag>>([| |])

    do 
        let raiseChanged () = _changed.Trigger this

        _markMap.MarkChanged
        |> Observable.subscribe (fun data -> raiseChanged())
        |> _eventHandlers.Add

    member x.GetTags span = 
        match _currentMark with
        | Some currentMark -> 
            match _markMap.GetMark currentMark _vimBufferData with
            | Some virtualPoint ->
                let point = virtualPoint.Position;
                let tag = MarkGlyphTag(currentMark.Char)
                let tagSpan = TagSpan(SnapshotSpan(point, 0), tag) :> ITagSpan<MarkGlyphTag>
                ReadOnlyCollectionUtil.Single tagSpan
            | None ->
                EmptyTagList
        | None -> 
    ...
    interface IBasicTaggerSource<MarkGlyphTag> with
        member x.GetTags span = x.GetTags span
        [<CLIEvent>]
        member x.Changed = _changed.Publish
    ...
    interface IViewTaggerProvider with 
        member x.CreateTagger<'T when 'T :> ITag> (textView: ITextView, textBuffer) = 
            if textView.TextBuffer = textBuffer then
                ...
                | Some vimBuffer ->
                    let func () = 
                        let taggerSource = new MarkGlyphTaggerSource(vimBuffer.VimBufferData)
                        taggerSource :> IBasicTaggerSource<MarkGlyphTag>
                    let tagger = TaggerUtil.CreateBasicTagger textView.Properties _key func
                    tagger :> obj :?> ITagger<'T>
            else
                null
