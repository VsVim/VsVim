...
type internal InsertUtil(_vimBufferData: IVimBufferData, _motionUtil: IMotionUtil, _operations: ICommonOperations) =
    ...
    member x.ClearReplaceUndoLine() = _replaceUndoLine <- None
    ...
    member x.CheckRecordReplaceUndoLine() =
        ...
        if recordLine then _replaceUndoLine <- SnapshotUtil.GetLine x.CurrentSnapshot x.CaretLineNumber |> Some
    ...
    member x.ApplyTextChangeCore (textEdit: ITextEdit) (position: int) (bounds: Span) (textChange: TextChange)
           addNewLines =
        ...
        let rec build textChange =
            ...
            | TextChange.DeleteRight count -> deleteRight.Value <- deleteRight.Value + count
        ...
        if not (StringUtil.IsNullOrEmpty insertText.Value) then
            if not (textEdit.Insert(position, insertText.Value)) then error <- true
        ...
        if deleteRight.Value > 0 then
            ...
            if not (textEdit.Delete(position, realCount)) then error <- true
        ...
        if deleteLeft.Value > 0 then
            ...
            let start, count =
                ...
                if diff < bounds.Start then bounds.Start, position - bounds.Start else diff, deleteLeft.Value

            if not (textEdit.Delete(start, count)) then error <- true

        if error then None else Some caretPosition.Value
    ...
    member x.ApplyBlockInsert (insertCommand: InsertCommand) padShortLines atEndOfLine startLineNumber spaces height =
        ...
        let fixOverlapEdits() =
            ...
            if textEdit.HasEffectiveChanges then textEdit.Apply() |> ignore else textEdit.Cancel()
        ...
        let doApply() =
            for i = 0 to height - 1 do
                ...
                let column =
                    if atEndOfLine
                    then SnapshotColumn.GetLineEnd(currentLine) |> Some
        ...
        if not atEndOfLine then fixOverlapEdits()
        doApply()

    /// Delete the character before the cursor
    member x.Back() = x.RunBackspacingCommand InsertCommand.Back
    ...
    member x.Combined left right =
        let clear =
            if Option.isNone _combinedEditStartPoint then
                _combinedEditStartPoint <-
                    x.CurrentSnapshot.CreateTrackingPoint(x.CaretPoint.Position, PointTrackingMode.Negative) |> Some
                true
            else
                false
        try
            ...
            if clear then _combinedEditStartPoint <- None
    ...
    member x.DeleteWordBeforeCursor() = x.RunBackspacingCommand InsertCommand.DeleteWordBeforeCursor
    ...
    member x.Replace(c: char) =
        ...
        let oldValue =
            EditorOptionsUtil.GetOptionValueOrDefault _editorOptions DefaultTextViewOptions.OverwriteModeId true
    ...
    member x.InsertCharacterAboveCaret() = x.InsertCharacterCore (x.CaretLineNumber - 1) false

    /// Insert the character immediately below the caret
    member x.InsertCharacterBelowCaret() = x.InsertCharacterCore (x.CaretLineNumber + 1) false

    /// Replace the character immediately above the caret
    member x.ReplaceCharacterAboveCaret() = x.InsertCharacterCore (x.CaretLineNumber - 1) true

    /// Replace the character immediately below the caret
    member x.ReplaceCharacterBelowCaret() = x.InsertCharacterCore (x.CaretLineNumber + 1) true
        ...
        x.EditWithUndoTransaction "New Line" (fun () ->
            ...
            | Some indent ->
                ...
                | Some newLine ->
                    ...
                    // into virtual space
                    let indentText =
                        StringUtil.Repeat indent " " |> (fun text -> _operations.NormalizeBlanks text 0)
    ...
    member x.InsertPreviouslyInsertedText stopInsert =
        ...
        | Some text -> _textBuffer.Insert(x.CaretPoint.Position, text) |> ignore
        | None -> ()

        if stopInsert then ModeSwitch.SwitchMode ModeKind.Normal else ModeSwitch.NoSwitch
    ...
    member x.InsertTab() =
        ...
        x.EditWithUndoTransaction "Insert Tab" (fun () ->
            ...
            let indentSpaces =
                if _localSettings.SoftTabStop <> 0 then _localSettings.SoftTabStop else _localSettings.TabStop
    ...
    member x.MoveCaretByWord direction =
        ...
        let success =
            ...
            | Direction.Left -> doMotion Motion.WordBackward (fun span -> span.Start)
            | Direction.Right -> doMotion Motion.WordForward (fun span -> span.End)
            | _ -> false
    ...
    member x.RepeatBlock (insertCommand: InsertCommand) (visualInsertKind: VisualInsertKind) (blockSpan: BlockSpan) =
        ...
        match isRepeatable,
              insertCommand.TextChange _editorOptions _textBuffer |> Option.map (fun textChange -> textChange.Reduce) with
            ...
            | TextChange.Insert text ->
                x.EditWithUndoTransaction "Repeat Block Edit" (fun () ->
                    ...
                    let padShortLines, atEndOfLine, spaces =
                        ...
                        | VisualInsertKind.Start -> false, false, blockSpan.BeforeSpaces
                        | VisualInsertKind.End -> true, false, blockSpan.BeforeSpaces + blockSpan.SpacesLength
                        | VisualInsertKind.EndOfLine -> false, true, blockSpan.BeforeSpaces
                    x.ApplyBlockInsert insertCommand padShortLines atEndOfLine startLineNumber spaces
                        (blockSpan.Height - 1)
    ...
    member x.RunInsertCommand command =
        if x.ShouldRunForEachCaret command
        then fun () -> x.RunInsertCommandCore command |> _operations.RunForAllSelections
        else x.RunInsertCommandCore command
    ...
    member x.RunInsertCommandCore command =
        ...
        if _vimHost.TryCustomProcess _textView command then
            ...
            | InsertCommand.BlockInsert(insertCommand, padShortLines, atEndOfLine, count) ->
                x.BlockInsert insertCommand padShortLines atEndOfLine count
    ...
    member x.ShiftLine(direction: int) =
        ...
        let column =
            (_operations.NormalizeBlanksToSpaces (indentSpan.GetText()) 0).Length + if isBlankLine
                                                                                       && x.CaretVirtualPoint.IsInVirtualSpace then
        ...
        let offset =
            ...
            if remainder = 0 then direction * _localSettings.ShiftWidth
            else if direction = -1 then -remainder
            else _localSettings.ShiftWidth - remainder
        ...
        x.EditWithUndoTransaction "Shift line" (fun () ->
            ...
            if isBlankLine then TextViewUtil.MoveCaretToPoint _textView x.CaretLine.End)

        CommandResult.Completed ModeSwitch.NoSwitch

    /// Shift the caret line one 'shiftwidth' to the left
    member x.ShiftLineLeft() = x.ShiftLine -1

    /// Shift the caret line one 'shiftwidth' to the right
    member x.ShiftLineRight() = x.ShiftLine 1
    ...
    member x.UndoReplace() =
        if x.CaretColumn.ColumnNumber > 0 then
            ...
            | Some codePoint ->
                ...
                TextViewUtil.MoveCaretToPosition _textView point.Position
        else if _globalSettings.IsBackspaceEol && x.CaretLineNumber > 0 then
    ...
    member x.DeleteLineBeforeCursor() = x.RunBackspacingCommand InsertCommand.DeleteLineBeforeCursor
    ...
    member x.RunBackspacingCommand command =
        ...
        | BackspaceCommand.None -> _operations.Beep()
    ...
    member x.AdjustBackspaceForStartSetting backspaceCommand =
        ...
        | false, Some startPoint, BackspaceCommand.Characters count ->
            ...
            if count = 0 then BackspaceCommand.None else BackspaceCommand.Characters count
    ...
    member x.BackspaceOverCharPoint() =
        ...
        if _localSettings.SoftTabStop <> 0 && SnapshotPointUtil.IsBlank prevPoint
           && _vimTextBuffer.IsSoftTabStopValidForBackspace then
    ...
    member x.BackspaceOverIndent() =
        ...
        if SnapshotPointUtil.IsChar '\t' prevPoint then
            ...
            let tabBoundarySpaces =
                ...
                if remainder = 0 then caretSpaces - _localSettings.SoftTabStop else caretSpaces - remainder
            ...
            if tabBoundarySpaces < 0 then
                ...
                if allBlank
                then BackspaceCommand.Characters(x.CaretPoint.Position - tabBoundaryColumn.StartPosition)
                else BackspaceCommand.Characters 1
