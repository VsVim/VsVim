...
function Update-VsixVersion() {
  if ($env:GITHUB_RUN_ID -eq $null) {
    throw "The environment variable %GITHUB_RUN_ID% is not set"
  }

  Write-Host "Updating VSIX version to include $($env:GITHUB_RUN_ID)"
  ...
  foreach ($vsVersion in $vsVersions) {
    ...
    $version = New-Object Version ([int]$version.Major),([int]$version.Minor),$env:GITHUB_RUN_ID
